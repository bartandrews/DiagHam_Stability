...
class ComputeHofstadterBands:
    ...
    def __init__(self, numCellX, numCellY, unitCellX, unitCellY, numFlux, gauge, t2,t3,gammaX,gammaY,alpha):
        print("ComputeHofstadterBands version 1.17")
        ...
        self.T1x = 1.0*alpha
        self.T1y = 1.0/float(alpha)
        self.T2x = t2*alpha*0.0
        self.T2y = t2/alpha
        self.T3 = t3
        self.GammaX = gammaX
        self.GammaY = gammaY
        ...
        for kx in range(0,self.NumCellX):
            for ky in range(0,self.NumCellY):
                ...
                if(index >= 0 and index < self.NumState):
                    ...
                    K1 = self.KXFactor * (kx +self.GammaX)
                    K2 = self.KYFactor * (ky+ self.GammaY)
                        ...
                        for i in range(0,self.UnitCellX):
                            ...
                            for j in range(0,self.UnitCellY):
                                ...
                                if(initIndex >= finalIndex):
                                   tmpHamiltonian[initIndex,finalIndex] += phase* self.T1x * xPhaseFactor.conjugate()
                                   if(initIndex != finalIndex):
                                        tmpHamiltonian[finalIndex,initIndex] += phase.conjugate()* self.T1x * xPhaseFactor

                                (finalIndex,phase) = self.GetIndexAndPhase(i-1,j,K1,K2)

                                if(initIndex >= finalIndex):
                                   tmpHamiltonian[initIndex,finalIndex] += phase* self.T1x * xPhaseFactor
                                   if(initIndex != finalIndex):
                                        tmpHamiltonian[finalIndex,initIndex] += phase.conjugate()* self.T1x * xPhaseFactor.conjugate()

                                (finalIndex,phase) = self.GetIndexAndPhase(i,j+1,K1,K2)

                                if(initIndex >= finalIndex):
                                    tmpHamiltonian[initIndex,finalIndex] += phase * yPhaseFactor.conjugate()* self.T1y
                                    if(initIndex != finalIndex):
                                        tmpHamiltonian[finalIndex,initIndex] += phase.conjugate() * yPhaseFactor* self.T1y

                                (finalIndex,phase) = self.GetIndexAndPhase(i,j-1,K1,K2)

                                if(initIndex >= finalIndex):
                                    tmpHamiltonian[initIndex,finalIndex] += phase * yPhaseFactor* self.T1y
                                    if(initIndex != finalIndex):
                                        tmpHamiltonian[finalIndex,initIndex] += phase.conjugate() * yPhaseFactor.conjugate() * self.T1y
                    ...
                    # Landau gauge in y-axis
                        ...
                        for i in range(0,self.UnitCellX):
                            ...
                            for j in range(0,self.UnitCellY):
                                ...
                                (finalIndex,phase) = self.GetIndexAndPhase(i+1,j,K1,K2)
                                if(initIndex >= finalIndex):
                                   tmpHamiltonian[initIndex,finalIndex] += phase* self.T1x
                                   if(initIndex != finalIndex):
                                        tmpHamiltonian[finalIndex,initIndex] += phase.conjugate()* self.T1x
                               ...
                               # modified Hofstadter with NNN hopping for simulating k^4 Hamiltonian:
                                ...
                                if(initIndex >= finalIndex):
                                   tmpHamiltonian[initIndex,finalIndex] += phase * self.T2x
                                   if(initIndex != finalIndex):
                                        tmpHamiltonian[finalIndex,initIndex] += phase.conjugate() * self.T2x
                                ...
                                if(initIndex >= finalIndex):
                                    tmpHamiltonian[initIndex,finalIndex] += phase * yPhaseFactor.conjugate()  * yPhaseFactor.conjugate() * self.T2y
                                    if(initIndex != finalIndex):
                                        tmpHamiltonian[finalIndex,initIndex] += phase.conjugate() * yPhaseFactor * yPhaseFactor * self.T2y

                                (finalIndex,phase) = self.GetIndexAndPhase(i,j-2,K1,K2)

                                if(initIndex >= finalIndex):
                                    tmpHamiltonian[initIndex,finalIndex] += phase * yPhaseFactor * yPhaseFactor * self.T2y
                                    if(initIndex != finalIndex):
                                        tmpHamiltonian[finalIndex,initIndex] += phase.conjugate() * yPhaseFactor.conjugate()  * yPhaseFactor.conjugate() * self.T2y
                                
                                #diagonal hoppings

                                (finalIndex,phase) = self.GetIndexAndPhase(i+1,j+1,K1,K2)

                                if(initIndex >= finalIndex):
                                    tmpHamiltonian[initIndex,finalIndex] += phase * yPhaseFactor * yPhaseFactor * self.T3
                                    if(initIndex != finalIndex):
                                        tmpHamiltonian[finalIndex,initIndex] += phase.conjugate() * yPhaseFactor.conjugate()  * yPhaseFactor.conjugate() * self.T3
