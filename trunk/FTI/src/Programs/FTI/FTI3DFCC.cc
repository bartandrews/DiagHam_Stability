...
{
    OptionManager Manager ("FTI3DFCC" , "0.01");
    ...
    (*SystemGroup) += new SingleDoubleOption  ('\n', "v-potential", "repulsive nearest neighbor potential strength", 1.0);
    ...
    (*SystemGroup) += new SingleDoubleOption  ('\n', "mu-s", "sublattice staggered chemical potential", 2.0);
    (*SystemGroup) += new SingleDoubleOption  ('\n', "flux", "kz flux", M_PI/8);
    ...
    {
        cout << "see man page for option syntax or type FTI3DFCC -h" << endl;
        return -1;
    }
    ...
    sprintf(FilePrefix, "%s_singleband_fcc_p_%d_x_%d_y_%d_z_%d_flux_%g", StatisticPrefix, NbrParticles, NbrSiteX, NbrSiteY, NbrSiteZ,
            Manager.GetDouble("flux"));
    ...
    if (Manager.GetBoolean("singleparticle-spectrum") == true)
    {
        ...
        {
            double chemicalPotentials[3] = {-Manager.GetDouble("mu-s"), 0.0, Manager.GetDouble("mu-s")};
            TightBindingModel = new TightBindingModel3DAtomicLimitLattice(NbrSiteX, NbrSiteY, NbrSiteZ, 3, chemicalPotentials,
                    Manager.GetDouble("gamma-x"), Manager.GetDouble("gamma-y"), Manager.GetDouble("gamma-z"), Architecture.GetArchitecture(), ExportOneBody);
        }
        else
            TightBindingModel = new TightBindingModelFCCLattice(NbrSiteX, NbrSiteY, NbrSiteZ, Manager.GetDouble("flux"), Manager.GetInteger("band-index"),
                    Manager.GetDouble("gamma-x"), Manager.GetDouble("gamma-y"), Manager.GetDouble("gamma-z"), Architecture.GetArchitecture(), ExportOneBody);
    ...
    for (int i = MinKx; i <= MaxKx; ++i)
    {
        for (int j = MinKy; j <= MaxKy; ++j)
        {
            for (int k = MinKz; k <= MaxKz; ++k)
            {
                ...
                Hamiltonian = new ParticleOnLatticeFCCLatticeSingleBandHamiltonian(Space, NbrParticles,
                        NbrSiteX, NbrSiteY, NbrSiteZ, Manager.GetDouble("u-potential"), Manager.GetDouble("v-potential"),
                        TightBindingModel, Manager.GetInteger("band-index"), Manager.GetBoolean("flat-band"), Architecture.GetArchitecture(), Memory);
