...
{
  cout.precision(14);
  OptionManager Manager ("HubbardHaldaneLatticeModel" , "0.01");
...
;
  ...
  if (Manager.GetBoolean("boson") == false)
      ...
      if (SzSymmetryFlag == false)
	{
	  if (GutzwillerFlag == false)
	    sprintf (StatisticPrefix, "fermions_hubbard_haldane");
	  else
	    sprintf (StatisticPrefix, "fermions_hubbard_haldane_gutzwiller");
	}
      else
	{
	  if (GutzwillerFlag == false)
	    sprintf (StatisticPrefix, "fermions_hubbard_haldane_szsym");
	  else
	    sprintf (StatisticPrefix, "fermions_hubbard_haldane_gutzwiller_szsym");
	}
    }
  else
    {
	...
	{
	  if (GutzwillerFlag == false)
	    sprintf (StatisticPrefix, "bosons_hubbard_haldane");
	  else
	    sprintf (StatisticPrefix, "bosons_hubbard_haldane_gutzwiller");
	}
      else
	{
	  if (GutzwillerFlag == false)
	    sprintf (StatisticPrefix, "bosons_hubbard_haldane_szsym");
	  else
	    sprintf (StatisticPrefix, "bosons_hubbard_haldane_gutzwiller_szsym");
	}
    }
  ...
  if (Manager.GetString("import-onebody") == 0)
    {
      TightBindingModel = new TightBindingModelHaldaneHoneycombLattice (NbrSitesX, NbrSitesY, Manager.GetDouble("t1"), Manager.GetDouble("t2"), 
									    HaldanePhi, Manager.GetDouble("mu-s"), Manager.GetDouble("gamma-x"), Manager.GetDouble("gamma-y"), Architecture.GetArchitecture(), true);
	  ...
	  for (int YMomentum = MinYMomentum; YMomentum <= MaxYMomentum; ++YMomentum)
	      ...
	      for (; SzParitySector <= MaxSzParitySector; SzParitySector += 2)
		{
		  ...
		  if (SzSymmetryFlag == false)
			...
			Space = new FermionOnLatticeWithSpinAndGutzwillerProjectionRealSpaceAnd2DTranslation (NbrParticles, TotalSz, NbrSites, XMomentum, NbrSitesX,   YMomentum, NbrSitesY);
		    }
		  else
		    {
		      bool MinusParitySector = true;
