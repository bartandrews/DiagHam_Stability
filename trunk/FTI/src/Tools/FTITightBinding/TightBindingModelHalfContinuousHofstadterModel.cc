...
TightBindingModelHalfContinuousHofstadterModel::TightBindingModelHalfContinuousHofstadterModel(char* fileName)
{
  ifstream File;
...
TightBindingModelHalfContinuousHofstadterModel::~TightBindingModelHalfContinuousHofstadterModel()
{
}
...
void TightBindingModelHalfContinuousHofstadterModel::CoreComputeBandStructure(long minStateIndex, long nbrStates)
{
  if (nbrStates == 0l)
  ...
  for (int kx = 0; kx < this->NbrSiteX ; ++kx)
    {
      for (int ky = 0; ky <  this->NbrSiteY; ++ky)
	      ...
	      for (int t = 0; t < this->NbrStep; t++)
		{
		  ...
		  int IntermediateIndex1 = this->GetIntermediateLinearizedIndices(t+1);
		  ...
		  IntermediateIndex1=this->GetIntermediateLinearizedIndices(t-1);
		  if (IntermediateIndex1 < IntermediateIndex)
		    TmpOneBodyHamiltonian.AddToMatrixElement(IntermediateIndex1, IntermediateIndex,-LaserStrength*Phase(-K2));                         
		}
...
bool TightBindingModelHalfContinuousHofstadterModel::WriteAsciiSpectrum(char* fileName)
{
  ofstream File;
  File.open(fileName);
  File.precision(10);
...
ofstream& TightBindingModelHalfContinuousHofstadterModel::WriteHeader(ofstream& output)
{
  int Dimension = 2;
