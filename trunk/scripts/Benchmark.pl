...
my @FermionTimeBipro;
while (defined($Line1 = <INFILE>) && defined($Line2 = <INFILE>) && defined($Line3 = <INFILE>))
  {
    chomp ($Line1);
    chomp ($Line2);
    chomp ($Line3);
	...
	if ($TmpArray[4] eq "mono")
	  {
	    &ParseResults ($TmpArray[3], $Line2, \%BosonSizeMono, \@BosonNbrPerSizeMono, \@BosonTimeMono);
	  }
	else
	  {
	    &ParseResults ($TmpArray[3], $Line2, \%BosonSizeBipro, \@BosonNbrPerSizeBipro, \@BosonTimeBipro);
	  }
      }
    else
      {
	if ($TmpArray[0] eq "fermions")
	  {
	    if ($TmpArray[4] eq "mono")
	      {
		&ParseResults ($TmpArray[3], $Line2, \%FermionSizeMono, \@FermionNbrPerSizeMono, \@FermionTimeMono);
	      }
	    else
	      {
		&ParseResults ($TmpArray[3], $Line2, \%FermionSizeBipro, \@FermionNbrPerSizeBipro, \@FermionTimeBipro);
	      }
	  }
      }
  }
...
sub ParseResults
  {
    ...
    $TmpArray[1] =~ s/system//;
    $TmpArray[2] =~ s/elapsed//;
    my @TmpArray2 = split (/\:/, $TmpArray[2]);
    my $RunTime = ($TmpArray2[0] * 60) + $TmpArray2[1] - $TmpArray[1];
    if (defined ($Position))
      {
	$$NbrPerId[$Position]++;
