...
#ifndef FERMIONONSPHEREWITHSPINLZSZSYMMETRY_H
#define FERMIONONSPHEREWITHSPINLZSZSYMMETRY_H
...
#define FERMION_SPHERE_SU2_LZ_SYMMETRIC_TEST   0xa000000000000000ul
#define FERMION_SPHERE_SU2_SZ_SYMMETRIC_TEST   0x6000000000000000ul
...
#define FERMION_SPHERE_SU2_LZ_SYMMETRIC_TEST   0xa0000000ul
#define FERMION_SPHERE_SU2_SZ_SYMMETRIC_TEST   0x60000000ul
...
class FermionOnSphereWithSpinLzSzSymmetry :  public FermionOnSphereWithSpin
{
 ...
 public:
  ...
  // default constructor 
  //
  FermionOnSphereWithSpinLzSzSymmetry ();
  ...
  FermionOnSphereWithSpinLzSzSymmetry(const FermionOnSphereWithSpinLzSzSymmetry& fermions);
  ...
  FermionOnSphereWithSpinLzSzSymmetry& operator = (const FermionOnSphereWithSpinLzSzSymmetry& fermions);
...
inline unsigned long FermionOnSphereWithSpinLzSzSymmetry::GetCanonicalState (unsigned long initialState)
{
...
inline unsigned long FermionOnSphereWithSpinLzSzSymmetry::GetSignedCanonicalState (unsigned long initialState)
{
  initialState <<= this->InvertShift;
#ifdef __64_BITS__
  unsigned long TmpState = FermionOnSphereWithSpinLzInvertTable[initialState & 0xff] << 56;
...
inline int FermionOnSphereWithSpinLzSzSymmetry::SymmetrizeAdAdResult(unsigned long& state, double& coefficient)
{
  unsigned long TmpState2 = state;
  state = this->GetSignedCanonicalState(state);
  switch (state & FERMION_SPHERE_SU2_FULLY_SYMMETRIC_BIT)
    {
    case 0x0ul:
      {
	...
	if (((1.0 - 2.0 * ((double) ((TmpState2 >> FERMION_SPHERE_SU2_SINGLETPARITY_SHIFT) & 0x1ul))) * this->SzParitySign) < 0.0)
	  return this->HilbertSpaceDimension;
	if ((this->ProdASignature & FERMION_SPHERE_SU2_FULLY_SYMMETRIC_BIT) != 0x0ul)
	  {
	    if ((this->ProdASignature & FERMION_SPHERE_SU2_FULLY_SYMMETRIC_BIT) == FERMION_SPHERE_SU2_FULLY_SYMMETRIC_BIT)
	      coefficient *= 2;
	    else
	      coefficient *= M_SQRT2;
	  }
	...
	coefficient *= 1.0 + ((double) (((state >> FERMION_SPHERE_SU2_SZ_TRANSFORMATION_SHIFT) 
					 | (state >> FERMION_SPHERE_SU2_LZ_TRANSFORMATION_SHIFT)) & 0x1ul)) * ((TmpSign * this->SzParitySign) - 1.0);
	if ((this->ProdASignature & FERMION_SPHERE_SU2_FULLY_SYMMETRIC_BIT) == 0x0ul)
	...
	if ((TmpSign * this->LzParitySign) < 0.0)
	  return this->HilbertSpaceDimension;
	coefficient *= 1.0 + ((double) ((state >> FERMION_SPHERE_SU2_SZ_TRANSFORMATION_SHIFT) & 0x1ul)) * ((TmpSign * this->SzParitySign) - 1.0);
	if ((this->ProdASignature & FERMION_SPHERE_SU2_FULLY_SYMMETRIC_BIT) == 0x0ul)
	  coefficient *= M_SQRT1_2;
	else
	  if ((this->ProdASignature & FERMION_SPHERE_SU2_FULLY_SYMMETRIC_BIT) == FERMION_SPHERE_SU2_FULLY_SYMMETRIC_BIT)
	...
	if ((TmpSign * this->SzParitySign) < 0.0)
	  return this->HilbertSpaceDimension;
	coefficient *= 1.0 + ((double) ((state >> FERMION_SPHERE_SU2_LZ_TRANSFORMATION_SHIFT) & 0x1ul)) * ((TmpSign * this->LzParitySign) - 1.0);
	if ((this->ProdASignature & FERMION_SPHERE_SU2_FULLY_SYMMETRIC_BIT) == 0x0ul)
	  coefficient *= M_SQRT1_2;
	else
	  if ((this->ProdASignature & FERMION_SPHERE_SU2_FULLY_SYMMETRIC_BIT) == FERMION_SPHERE_SU2_FULLY_SYMMETRIC_BIT)
