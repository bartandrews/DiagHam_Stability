...
#ifndef BOSONONLATTICEKYLONG_H
#define BOSONONLATTICEKYLONG_H
...
class BosonOnLatticeKyLong : public ParticleOnLattice
{
 ...
 protected:
  ...
  // tempory items used in conversion to full n-body basis:
 ...
 public:
  ...
  BosonOnLatticeKyLong (int nbrBosons, int lx, int ly, int ky, int nbrFluxQuanta, unsigned long memory = 10000000);


  // copy constructor (without duplicating datas)
  //
  // bosons = reference on the hilbert space to copy to copy
  BosonOnLatticeKyLong (const BosonOnLatticeKyLong& bosons);

  // destructor
  //
  virtual ~BosonOnLatticeKyLong ();

  // assignement (without duplicating datas)
  //
  // bosons = reference on the hilbert space to copy to copy
  // return value = reference on current hilbert space
  BosonOnLatticeKyLong& operator = (const BosonOnLatticeKyLong& bosons);
  ...
  virtual long unsigned int Ad (long unsigned int state, int q, double& coefficient);
 ...
 protected:
  ...
  int FindStateIndex(ULONGLONG stateDescription, int highestBit);
  ...
  void ExpandBasisState (int nbrOperators, int *quantumNumbers, ULONGLONG state, Complex prefactor);
  ...
  // return value = corresponding fermionic state  
  ULONGLONG BosonToFermion(unsigned long*& initialState, int& initialStateLzMax, int &finalStateHighestBit);
...
inline int BosonOnLatticeKyLong::GetParticleStatistic()
{
...
//
// return value = particle statistic
inline char BosonOnLatticeKyLong::GetLandauGaugeAxis()
{
  return 'y';
}
...
inline ULONGLONG BosonOnLatticeKyLong::BosonToFermion(unsigned long*& initialState, int& initialStateLzMax, int &finalStateHighestBit)
{
...
inline ULONGLONG BosonOnLatticeKyLong::BosonToFermion(unsigned long*& initialState, int& initialStateLzMax)
{
...
inline void BosonOnLatticeKyLong::FermionToBoson(ULONGLONG initialState, int initialStateLzMax, unsigned long*& finalState, int& finalStateLzMax)
{
  finalStateLzMax = 0;
