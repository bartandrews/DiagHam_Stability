...
BosonOnSquareLatticeMultiBandMomentumSpace::BosonOnSquareLatticeMultiBandMomentumSpace ()
{
}
...
BosonOnSquareLatticeMultiBandMomentumSpace::BosonOnSquareLatticeMultiBandMomentumSpace (int nbrBosons, int nbrSiteX, int nbrSiteY, int kxMomentum, int kyMomentum, int nbrBand, unsigned long memory)
{  
  ...
  this->NbrLzValue = 2*this->LzMax + 1;
  this->Minors = 0;
  this->KeptCoordinates = 0;
  this->NbrBand = nbrBand;
  //this->TargetSpace = this;
  this->TemporaryState = new unsigned long [2*this->NbrLzValue];
  this->ProdATemporaryState = new unsigned long [2*this->NbrLzValue];
  this->LargeHilbertSpaceDimension = this->EvaluateHilbertSpaceDimension(this->NbrBosons, this->NbrSiteX - 1, this->NbrSiteY - 1, 0, 0, this->NbrBand - 1);
  ...
  if ( this->LargeHilbertSpaceDimension > 0l)
    {
      ...
      long TmpLargeHilbertSpaceDimension = this->GenerateStates(TmpStateDescription, this->NbrBosons, this->NbrSiteX - 1, this->NbrSiteY - 1, 0, 0, 2*this->LzMax + this->NbrBosons, 0l, this->NbrBand - 1);
      if (TmpLargeHilbertSpaceDimension != this->LargeHilbertSpaceDimension)
	{
      ...
      this->FermionBasis = new FermionOnSphere(this->NbrBosons, 0, 2*this->LzMax + this->NbrBosons - 1, TmpStateDescription, this->LargeHilbertSpaceDimension);
#ifdef __DEBUG__
      long UsedMemory = 0;
...
BosonOnSquareLatticeMultiBandMomentumSpace::BosonOnSquareLatticeMultiBandMomentumSpace(const BosonOnSquareLatticeMultiBandMomentumSpace& bosons)
{
...
BosonOnSquareLatticeMultiBandMomentumSpace& BosonOnSquareLatticeMultiBandMomentumSpace::operator = (const BosonOnSquareLatticeMultiBandMomentumSpace& bosons)
{
  if (bosons.TargetSpace != &bosons)
    this->TargetSpace = bosons.TargetSpace;
  else
...
AbstractHilbertSpace* BosonOnSquareLatticeMultiBandMomentumSpace::Clone()
{
  return new BosonOnSquareLatticeMultiBandMomentumSpace(*this);
}
...
bool BosonOnSquareLatticeMultiBandMomentumSpace::WriteHilbertSpace (char* fileName)
{
  ofstream File;
...
void BosonOnSquareLatticeMultiBandMomentumSpace::GetOccupied(int state, int* orbitals)
{
...
ostream& BosonOnSquareLatticeMultiBandMomentumSpace::PrintState (ostream& Str, int state)
{
  this->FermionToBoson(this->FermionBasis->StateDescription[state], this->FermionBasis->StateLzMax[state], this->TemporaryState, this->TemporaryStateLzMax);
  std::cout << "FermionBasis->StateDescription[state]: " << this->FermionBasis->StateDescription[state] << endl;
  //std::cout << "TemporaryStateLzMax: " << this->TemporaryStateLzMax << endl;
  Str << "[";
...
long BosonOnSquareLatticeMultiBandMomentumSpace::GenerateStates(unsigned long* stateDescription, int nbrBosons, int currentKx, int currentKy, int currentTotalKx, int currentTotalKy, int currentFermionicPosition, long pos, int band)
{
  if (band < 0)
  {
    return pos;
  }
  //std::cout << "===== Calling GenerateStates =====\n";
  if (nbrBosons < 0)
  {
    //std::cout << "..nbrBosons < 0\n";
    return pos;
  }
  if (currentKy < 0)
  {
  ...
  if (currentKx < 0)
  {
    currentKx = this->NbrSiteX - 1;
    --band;
  }

  /*std::cout << "nbrBosons: " << nbrBosons << endl;
  //std::cout << "currentKx: " << currentKx << endl;
  std::cout << "currentKy: " << currentKy << endl;
  std::cout << "currentTotalKx: " << currentTotalKx << endl;
  std::cout << "currentTotalKy: " << currentTotalKy << endl;
  std::cout << "pos: " << pos << endl;*/
  for (int k = nbrBosons; k > 0; --k)
    {
      //std::cout << "\tk: " << k << endl; 
      long TmpPos = this->GenerateStates(stateDescription, nbrBosons - k, currentKx, currentKy - 1, currentTotalKx + (k * currentKx), currentTotalKy + (k * currentKy), currentFermionicPosition - k - 1, pos, band);
      unsigned long Mask = ((0x1ul << k) - 0x1ul) << (currentFermionicPosition - k - 1);
      //std::cout << "\tMask: " << Mask << endl;
      for (; pos < TmpPos; ++pos)
      {
        //std::cout << "stateDescription[" << pos << "]: " << stateDescription[pos] << endl;
        stateDescription[pos] |= Mask;
        //std::cout << "stateDescription[" << pos << "]: " << stateDescription[pos] << endl; 
      }
       
    }
  return this->GenerateStates(stateDescription, nbrBosons, currentKx, currentKy - 1, currentTotalKx, currentTotalKy, currentFermionicPosition - 1, pos, band);
};
...
long BosonOnSquareLatticeMultiBandMomentumSpace::EvaluateHilbertSpaceDimension(int nbrBosons, int currentKx, int currentKy, int currentTotalKx, int currentTotalKy, int band)
{
  if (nbrBosons < 0)
    return 0l;
  if (band < 0)
  ...
  long Count = 0;
  for (int k = nbrBosons; k >= 0; --k)
    Count += this->EvaluateHilbertSpaceDimension(nbrBosons - k, currentKx, currentKy - 1, currentTotalKx + (k * currentKx), currentTotalKy + (k * currentKy), band);
  return Count;
}
...
/*
HermitianMatrix BosonOnSquareLatticeMultiBandMomentumSpace::EvaluatePartialDensityMatrixMomentumSpace (int subsytemSizeX, int subsytemSizeY, int subsytemStartX, int subsytemStartY, int nbrParticleSector, int kxSector, int kySector, ComplexVector& groundState)
{
...
long BosonOnSquareLatticeMultiBandMomentumSpace::EvaluatePartialDensityMatrixMomentumSpaceCore (int minIndex, int nbrIndex, ParticleOnSphere* complementaryHilbertSpace,  ParticleOnSphere* destinationHilbertSpace, ComplexVector& groundState,  HermitianMatrix* densityMatrix)
{
//   BosonOnSquareLatticeMultiBandMomentumSpace* TmpHilbertSpace =  (BosonOnSquareLatticeNonPeriodicMomentumSpace*) complementaryHilbertSpace;
//   BosonOnSquareLatticeMultiBandMomentumSpace* TmpDestinationHilbertSpace =  (BosonOnSquareLatticeNonPeriodicMomentumSpace*) destinationHilbertSpace;
...
ComplexMatrix BosonOnSquareLatticeMultiBandMomentumSpace::EvaluatePartialEntanglementMatrixMomentumSpace (int subsytemSizeX, int subsytemSizeY, int nbrParticleSector, int kxSector, int kySector, ComplexVector& groundState)
{
...
ComplexMatrix BosonOnSquareLatticeMultiBandMomentumSpace::EvaluatePartialEntanglementMatrixParticlePartition (int nbrParticleSector, int kxSector, int kySector, ComplexVector& groundState, bool removeBinomialCoefficient)
{
...
HermitianMatrix BosonOnSquareLatticeMultiBandMomentumSpace::EvaluatePartialDensityMatrixParticlePartition (int nbrParticleSector, int kxSector, int kySector, ComplexVector& groundState, AbstractArchitecture* architecture)
{
  ...
  BosonOnSquareLatticeMultiBandMomentumSpace SubsytemSpace (nbrParticleSector, this->NbrSiteX, this->NbrSiteY, kxSector, kySector);
  HermitianMatrix TmpDensityMatrix (SubsytemSpace.GetHilbertSpaceDimension(), true);
  BosonOnSquareLatticeMultiBandMomentumSpace ComplementarySpace (ComplementaryNbrParticles, this->NbrSiteX, this->NbrSiteY, ComplementaryKxMomentum, ComplementaryKyMomentum);
...
long BosonOnSquareLatticeMultiBandMomentumSpace::EvaluatePartialDensityMatrixParticlePartitionCore (int minIndex, int nbrIndex, ParticleOnSphere* complementaryHilbertSpace,  ParticleOnSphere* destinationHilbertSpace,
											   ComplexVector& groundState,  HermitianMatrix* densityMatrix)
{
  BosonOnSquareLatticeMultiBandMomentumSpace* TmpHilbertSpace =  (BosonOnSquareLatticeMultiBandMomentumSpace*) complementaryHilbertSpace;
  BosonOnSquareLatticeMultiBandMomentumSpace* TmpDestinationHilbertSpace =  (BosonOnSquareLatticeMultiBandMomentumSpace*) destinationHilbertSpace;
...
int BosonOnSquareLatticeMultiBandMomentumSpace::FindStateIndexFromArray(int* stateDescription)
{
...
HermitianMatrix BosonOnSquareLatticeMultiBandMomentumSpace::EvaluatePartialDensityMatrixParticlePartition (int nbrParticleSector, int kxSector, int kySector, 
...
ComplexVector BosonOnSquareLatticeMultiBandMomentumSpace::SymmetrizeU1U1State (ComplexVector& leftVector, ComplexVector& rightVector, BosonOnSquareLatticeMultiBandMomentumSpace* leftSpace, BosonOnSquareLatticeMultiBandMomentumSpace* rightSpace, bool unnormalizedBasisFlag, AbstractArchitecture* architecture)
{
  ComplexVector SymmetrizedVector (this->LargeHilbertSpaceDimension,true);

  FQHESquareLatticeMultiBandSymmetrizeU1U1StateOperation Operation (this, leftSpace, rightSpace, &SymmetrizedVector, &leftVector, &rightVector, unnormalizedBasisFlag);
  Operation.ApplyOperation(architecture);
...
void BosonOnSquareLatticeMultiBandMomentumSpace::SymmetrizeU1U1StateCore (ComplexVector& symmetrizedVector, ComplexVector& leftVector, ComplexVector& rightVector, BosonOnSquareLatticeMultiBandMomentumSpace* leftSpace, BosonOnSquareLatticeMultiBandMomentumSpace* rightSpace, bool unnormalizedBasisFlag, unsigned long firstComponent, unsigned long nbrComponents)
{
...
/* ------------- Attempt to reimplement creation and annihilation operators for multiband cases ----- */
...
int BosonOnSquareLatticeMultiBandMomentumSpace::AdAdAA (int index, int m1, int m2, int n1, int n2, double& coefficient)
{
...
int BosonOnSquareLatticeMultiBandMomentumSpace::ProdAdProdA (int index, int* m, int* n, int nbrIndices, double& coefficient)
{
  this->FermionToBoson(this->FermionBasis->StateDescription[index], this->FermionBasis->StateLzMax[index], this->TemporaryState, this->TemporaryStateLzMax);
  --nbrIndices;
...
double BosonOnSquareLatticeMultiBandMomentumSpace::AA (int index, int n1, int n2)
{
...
double BosonOnSquareLatticeMultiBandMomentumSpace::ProdA (int index, int* n, int nbrIndices)
{
  this->FermionToBoson(this->FermionBasis->StateDescription[index], this->FermionBasis->StateLzMax[index], this->ProdATemporaryState, this->ProdATemporaryStateLzMax);
...
double BosonOnSquareLatticeMultiBandMomentumSpace::ProdAL (int index, int* n, int nbrIndices)
{
  this->FermionToBoson(this->FermionBasis->StateDescription[index], this->FermionBasis->StateLzMax[index], this->ProdATemporaryState, this->ProdATemporaryStateLzMax);
...
int BosonOnSquareLatticeMultiBandMomentumSpace::AdAd (int m1, int m2, double& coefficient)
{
  for (int i = 0; i < this->NbrLzValue; ++i)
  {
    this->TemporaryState[i] = this->ProdATemporaryState[i];
    //std::cout << "this->TemporaryState[i] " << this->TemporaryState[i] << endl;
  }
  ++this->TemporaryState[m2];
  coefficient = this->TemporaryState[m2];
  ++this->TemporaryState[m1];
  coefficient *= this->TemporaryState[m1];
  coefficient = sqrt(coefficient);
  this->TemporaryStateLzMax = 2*this->LzMax;
  while (this->TemporaryState[this->TemporaryStateLzMax] == 0)
    --this->TemporaryStateLzMax;
  return this->FermionBasis->FindStateIndex(this->BosonToFermion(this->TemporaryState, this->TemporaryStateLzMax), this->TemporaryStateLzMax + this->NbrBosons - 1);
}
...
int BosonOnSquareLatticeMultiBandMomentumSpace::ProdAd (int* m, int nbrIndices, double& coefficient)
{
  for (int i = 0; i < this->NbrLzValue; ++i)
    this->TemporaryState[i] = this->ProdATemporaryState[i];
  int TmpCoefficient = 1;
...
int BosonOnSquareLatticeMultiBandMomentumSpace::ProdAdL (int* m, int nbrIndices, double& coefficient)
{
  for (int i = 0; i < this->NbrLzValue; ++i)
    this->TemporaryState[i] = this->ProdATemporaryState[i];
  double TmpCoefficient = 1.0;
...
double BosonOnSquareLatticeMultiBandMomentumSpace::AdA (int index, int m)
{
  this->FermionToBoson(this->FermionBasis->StateDescription[index], this->FermionBasis->StateLzMax[index], this->TemporaryState, this->TemporaryStateLzMax);
...
double BosonOnSquareLatticeMultiBandMomentumSpace::AdA (long index, int m)
{
  this->FermionToBoson(this->FermionBasis->StateDescription[index], this->FermionBasis->StateLzMax[index], this->TemporaryState, this->TemporaryStateLzMax);
  if (this->TemporaryStateLzMax < m)  
    return 0.0;
...
int BosonOnSquareLatticeMultiBandMomentumSpace::AdA (int index, int m, int n, double& coefficient)
{
  this->FermionToBoson(this->FermionBasis->StateDescription[index], this->FermionBasis->StateLzMax[index], this->TemporaryState, this->TemporaryStateLzMax);
