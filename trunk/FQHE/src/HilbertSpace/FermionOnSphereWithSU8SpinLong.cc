...
//                       for systems with more than 8 orbitals                //
//                                                                            //
...
FermionOnSphereWithSU8SpinLong::FermionOnSphereWithSU8SpinLong()
{
...
FermionOnSphereWithSU8SpinLong::FermionOnSphereWithSU8SpinLong (int nbrFermions, int totalLz, int lzMax, int* nbrParticleSigma,
							unsigned long memory)
{
  cout << "FermionOnSphereWithSU8SpinLong is some heavily untested code, use at your risk and debug it" << endl;
  ...
  if (TmpHilbertSpaceDimension != this->LargeHilbertSpaceDimension)
    {
       ...
       cout << "Mismatch in State-count and State Generation in FermionOnSphereWithSU8SpinLong!" << endl;
       exit(1);
     }
...
FermionOnSphereWithSU8SpinLong::FermionOnSphereWithSU8SpinLong(const FermionOnSphereWithSU8SpinLong& fermions)
{
...
FermionOnSphereWithSU8SpinLong& FermionOnSphereWithSU8SpinLong::operator = (const FermionOnSphereWithSU8SpinLong& fermions)
{
  if ((this->HilbertSpaceDimension != 0) && (this->Flag.Shared() == false) && (this->Flag.Used() == true))
    {
      delete[] this->StateDescription;
...
AbstractHilbertSpace* FermionOnSphereWithSU8SpinLong::Clone()
{
  return new FermionOnSphereWithSU8SpinLong(*this);
}
...
List<AbstractQuantumNumber*> FermionOnSphereWithSU8SpinLong::GetQuantumNumbers ()
{
...
AbstractQuantumNumber* FermionOnSphereWithSU8SpinLong::GetQuantumNumber (int index)
{
...
AbstractHilbertSpace* FermionOnSphereWithSU8SpinLong::ExtractSubspace (AbstractQuantumNumber& q, 
...
int FermionOnSphereWithSU8SpinLong::FindStateIndex(ULONGLONG stateDescription, int lzmax)
{
...
ostream& FermionOnSphereWithSU8SpinLong::PrintState (ostream& Str, int state)
{
  ...
  for (int i = this->NbrLzValue-1; i >=0 ; --i)
    {
      Tmp = ((TmpState >> (i << 3)) & ((ULONGLONG) 0xfful));
      for (int j = 7; j >= 0; --j)
	{
	  if (Tmp & (((ULONGLONG) 0x1ul) << 7))
	    {
	      Str << j << " ";
	    }
	  else
...
long FermionOnSphereWithSU8SpinLong::GenerateStates(int nbrFermions, int lzMax, int totalLz, int nbrParticles1, int nbrParticles2,
  ...
  if (nbrFermions == 1) 
    {
      if (lzMax >= totalLz)
	{
	  this->StateDescription[pos] = ((ULONGLONG) (nbrParticles1 | (nbrParticles2 << 1) | (nbrParticles3 << 2) | (nbrParticles4 << 3)
...
void FermionOnSphereWithSU8SpinLong::GenerateLookUpTable(unsigned long memory)
{
...
long FermionOnSphereWithSU8SpinLong::ShiftedEvaluateHilbertSpaceDimension(int nbrFermions, int lzMax, int totalLz, int nbrParticles1, int nbrParticles2,
								      int nbrParticles3, int nbrParticles4, int nbrParticles5, int nbrParticles6,
...
Complex FermionOnSphereWithSU8SpinLong::EvaluateWaveFunction (RealVector& state, RealVector& position, AbstractFunctionBasis& basis,
							    int firstComponent, int nbrComponent)
{
  ...
  for (int k = firstComponent; k < LastComponent; ++k)
	...
	{
	  if ((TmpStateDescription & ((ULONGLONG) 0x3ul)) != ((ULONGLONG) 0x0ul))
	    {
	      Indices[Pos] = Lz;
	      ++Pos;
	    }
...
void FermionOnSphereWithSU8SpinLong::InitializeWaveFunctionEvaluation (bool timeCoherence)
{
}
...
RealVector FermionOnSphereWithSU8SpinLong::ForgeU1FromSU8(RealVector& state, FermionOnSphere& u1Space)
{
  ...
  for (int j = 0; j < this->HilbertSpaceDimension; ++j)    
	...
	{ 
	  TmpPos = 0;
	  TmpState2 = ((ULONGLONG) 0x0ul); 
	  while (TmpPos <= this->LzMax)
	    {
	      TmpState2 |= ((TmpState & ((ULONGLONG) 0x1ul)) | ((TmpState & ((ULONGLONG) 0x2ul)) >> 1) | ((TmpState & ((ULONGLONG) 0x4ul)) >> 2) | ((TmpState & ((ULONGLONG) 0x8ul)) >> 3)
			    | ((TmpState & ((ULONGLONG) 0x10ul)) >> 4) | ((TmpState & ((ULONGLONG) 0x20ul)) >> 5) | ((TmpState & ((ULONGLONG) 0x40ul)) >> 6) | ((TmpState & ((ULONGLONG) 0x80ul)) >> 7)) << TmpPos;
	      TmpState >>= 8;
	      ++TmpPos;
	    }
	  while ((TmpState2 >> TmpPos) == ((ULONGLONG) 0x0ul))
	    --TmpPos;
...
RealVector FermionOnSphereWithSU8SpinLong::ForgeSU2FromSU8(RealVector& state, FermionOnSphereWithSpin& su2Space)
{
  ...
  for (int j = 0; j < this->HilbertSpaceDimension; ++j)    
    {
      ...
      while (TmpPos >=0)
	{
	  if ((((TmpState2 >> 3) & (TmpState2 >> 2) & ((ULONGLONG) 0x1ul)) != ((ULONGLONG) 0x0ul)) ||
	      (((TmpState2 >> 1) & TmpState2 & ((ULONGLONG) 0x1ul)) != ((ULONGLONG) 0x0ul)))
	    TmpPos = 1;
	  TmpState2 >>= 8;
	  TmpPos -= 8;
	}
...
void FermionOnSphereWithSU8SpinLong::TransformOneBodyBasis(ComplexVector& initialState, ComplexVector& targetState, ComplexMatrix* oneBodyBasis, long firstComponent, long nbrComponents)
{
  ...
  for (long i = firstComponent; i < LastComponent; ++i)
	...
	{
	  Tmp = (TmpState >> (j << 3)) & ((ULONGLONG) 0xfful);
...
ComplexMatrix FermionOnSphereWithSU8SpinLong::TransformationMatrixOneBodyBasis(ComplexMatrix* oneBodyBasis)
{
  int* TmpMomentumIndices = new int [this->NbrFermions];
  int* TmpSU8Indices = new int [this->NbrFermions];
  int* TmpSU8Indices2 = new int [this->NbrFermions];
...
void FermionOnSphereWithSU8SpinLong::TransformOneBodyBasisRecursive(ComplexVector& targetState, Complex coefficient,
  ...
  if (position == this->NbrFermions)
    {
      ...
      ULONGLONG TmpState2;
      ULONGLONG Mask = ((ULONGLONG) 0x0ul);
      ULONGLONG MaskSign = ((ULONGLONG) 0x0ul);
      for (int i = 0; i < this->NbrFermions; ++i)
	{
	  Mask = ((ULONGLONG) 0x1ul) << ((momentumIndices[i] << 3) + currentSU8Indices[i]);
	  if ((TmpState & Mask) != ((ULONGLONG) 0x0ul))
	    return;
	  TmpState2 = TmpState & (Mask - ((ULONGLONG) 0x1ul));
#ifdef __128_BIT_LONGLONG__
	  TmpState2 ^= (TmpState2 >> 64);
#endif
	  ...
	  MaskSign ^= (TmpState2 ^ (TmpState2 >> 1)) & ((ULONGLONG) 0x1ul);
	  TmpState |= Mask;
	}
      int TmpLzMax = this->NbrLzValue << 3;
...
ComplexMatrix FermionOnSphereWithSU8SpinLong::TransformationMatrixSU8ToSU2(ParticleOnSphereWithSpin* targetSpace, int spinUp, int spinDown)
{
  ...
  ULONGLONG MaskUp = ((ULONGLONG) 0x1ul) << spinUp;
  ULONGLONG MaskDown = ((ULONGLONG) 0x1ul) << spinDown;
  ULONGLONG Mask = (~(MaskUp | MaskDown)) & ((ULONGLONG) 0xfful);
  double ReorderingSign = 1.0;
  if (spinUp > spinDown)
    ReorderingSign = -1.0;
  for (int i = 0; i < this->HilbertSpaceDimension; ++i)
    {
      ULONGLONG TmpState1 = this->StateDescription[i];
      ULONGLONG TmpState2 = ((ULONGLONG) 0x0ul);
      bool Flag = true;
      double Sign = 1.0;
      for (int j = 0; (j <= this->LzMax) && (Flag == true); ++j)
	{
	  if ((TmpState1 & (Mask << (j << 3))) == ((ULONGLONG) 0x0ul))
	    {
	      if ((TmpState1 & (MaskUp << (j << 3))) != ((ULONGLONG) 0x0ul))
		{
		  if ((TmpState1 & (MaskDown << (j << 3))) != ((ULONGLONG) 0x0ul))
		    {
		      TmpState2 |= ((ULONGLONG) 0x3ul) << (j << 1);
		      Sign *= ReorderingSign;
		    }
		  else
		    {
		      TmpState2 |= ((ULONGLONG) 0x1ul) << (j << 1);
		    }
		}
	      else
		{
      ...
      if (Flag == true)
	{
	  ...
	  while ((TmpState2 >> TmpLzMax) == ((ULONGLONG) 0x0ul))
	    --TmpLzMax;
