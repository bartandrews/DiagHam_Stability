...
#ifndef FERMIONONLATTICE_H
#define FERMIONONLATTICE_H
...
class FermionOnLattice :  public ParticleOnLattice
{
 ...
 public:
  ...
  FermionOnLattice (int nbrFermions, int lx, int ly, int nbrFluxQuanta, unsigned long memory = 10000000, double solenoidX=0.0, double solenoidY=0.0, int nbrSublattices=1, bool verbose=true);  


  // copy constructor (without duplicating datas)
  //
  // bosons = reference on the hilbert space to copy to copy
  FermionOnLattice(const FermionOnLattice& bosons);

  // virtual destructor
  //
  virtual ~FermionOnLattice ();

  // assignment (without duplicating datas)
  //
  // bosons = reference on the hilbert space to copy to copy
  // return value = reference on current hilbert space
  FermionOnLattice& operator = (const FermionOnLattice& bosons);
  ...
  virtual int GetMinNbrParticles() {return this->NbrFermions;}
