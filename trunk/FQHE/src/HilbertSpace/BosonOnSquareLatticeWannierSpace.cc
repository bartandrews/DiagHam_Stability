...
BosonOnSquareLatticeWannierSpace::BosonOnSquareLatticeWannierSpace ()
{
}
...
BosonOnSquareLatticeWannierSpace::BosonOnSquareLatticeWannierSpace(const BosonOnSquareLatticeWannierSpace& bosons)
{
...
BosonOnSquareLatticeWannierSpace& BosonOnSquareLatticeWannierSpace::operator = (const BosonOnSquareLatticeWannierSpace& bosons)
{
  if (bosons.TargetSpace != &bosons)
    this->TargetSpace = bosons.TargetSpace;
  else
...
AbstractHilbertSpace* BosonOnSquareLatticeWannierSpace::Clone()
{
  return new BosonOnSquareLatticeWannierSpace(*this);
}
...
bool BosonOnSquareLatticeWannierSpace::WriteHilbertSpace (char* fileName)
{
  ofstream File;
...
ostream& BosonOnSquareLatticeWannierSpace::PrintState (ostream& Str, int state)
{
  ...
  for (int i = this->TemporaryStateLzMax+1; i<LzMax; ++i)
    {
      Str <<"0 ";
    }
  Str << "]";
...
int BosonOnSquareLatticeWannierSpace::GetLinearizedMomentum(int state)
{
  this->FermionToBoson(this->FermionBasis->StateDescription[state], this->FermionBasis->StateLzMax[state], this->TemporaryState, this->TemporaryStateLzMax);

  int Result=0;
  for (int i = 1; i <= this->TemporaryStateLzMax; ++i)
    Result += i*this->TemporaryState[i];
...
// get total momentum in the linearized momentum index (moduly N_phi)
// state = index of state to operate on
int BosonOnSquareLatticeWannierSpace::ProjectToTorus(ParticleOnTorus *torusSpace, int state)
{
  //cout << "Torus projection "<<state<<endl;
  return ((BosonOnTorusShort*)torusSpace)->FindStateIndex(FermionBasis->StateDescription[state], this->FermionBasis->StateLzMax[state]);
}
...
long BosonOnSquareLatticeWannierSpace::GenerateStates(unsigned long* stateDescription, int nbrBosons, int currentKx, int currentKy, int currentTotalKy, int currentFermionicPosition, long pos)
{
  ...
  if (nbrBosons == 0)
    {
      if (((currentTotalKy % this->NbrSiteY) == this->KyMomentum))
	{
  ...
  for (int k = nbrBosons; k > 0; --k)
    {
      long TmpPos = this->GenerateStates(stateDescription, nbrBosons - k, currentKx, currentKy - 1, currentTotalKy + (k * currentKy), currentFermionicPosition - k - 1, pos);
...
long BosonOnSquareLatticeWannierSpace::EvaluateHilbertSpaceDimension(int nbrBosons, int currentKx, int currentKy, int currentTotalKy)
{
  if (nbrBosons < 0)
    return 0l;
  if (currentKy < 0)
...
HermitianMatrix BosonOnSquareLatticeWannierSpace::EvaluatePartialDensityMatrixWannierSpace (int subsytemSizeX, int subsytemSizeY, int subsytemStartX, int subsytemStartY, int nbrParticleSector, int kxSector, int kySector, ComplexVector& groundState)
{
...
//   BosonOnSquareLatticeNonPeriodicWannierSpace SubsytemSpace (nbrParticleSector, this->NbrSiteX, this->NbrSiteY, subsytemSizeX, subsytemSizeY, subsytemStartX, subsytemStartY, kxSector, kySector);
//   BosonOnSquareLatticeNonPeriodicWannierSpace ComplementarySpace (ComplementaryNbrParticles, this->NbrSiteX, this->NbrSiteY, ComplementarySubsytemSizeX, ComplementarySubsytemSizeY, ComplementarySubsytemStartX, ComplementarySubsytemStartY, ComplementaryKxMomentum, ComplementaryKyMomentum);
...
//       int nbrNonZeroElements = this->EvaluatePartialDensityMatrixWannierSpaceCore (0, ComplementarySpace.HilbertSpaceDimension, &ComplementarySpace, &SubsytemSpace, groundState, &TmpDensityMatrix);
...
long BosonOnSquareLatticeWannierSpace::EvaluatePartialDensityMatrixWannierSpaceCore (int minIndex, int nbrIndex, ParticleOnSphere* complementaryHilbertSpace,  ParticleOnSphere* destinationHilbertSpace, ComplexVector& groundState,  HermitianMatrix* densityMatrix)
{
//   BosonOnSquareLatticeWannierSpace* TmpHilbertSpace =  (BosonOnSquareLatticeNonPeriodicWannierSpace*) complementaryHilbertSpace;
//   BosonOnSquareLatticeWannierSpace* TmpDestinationHilbertSpace =  (BosonOnSquareLatticeNonPeriodicWannierSpace*) destinationHilbertSpace;
...
ComplexMatrix BosonOnSquareLatticeWannierSpace::EvaluatePartialEntanglementMatrixWannierSpace (int subsytemSizeX, int subsytemSizeY, int nbrParticleSector, int kxSector, int kySector, ComplexVector& groundState)
{
...
ComplexMatrix BosonOnSquareLatticeWannierSpace::EvaluatePartialEntanglementMatrixParticlePartition (int nbrParticleSector, int kxSector, int kySector, ComplexVector& groundState, bool removeBinomialCoefficient)
{
...
HermitianMatrix BosonOnSquareLatticeWannierSpace::EvaluatePartialDensityMatrixParticlePartition (int nbrParticleSector, int kxSector, int kySector, ComplexVector& groundState, AbstractArchitecture* architecture)
{
  if (nbrParticleSector == 0)
    {
      if (((kxSector == 0) && (kySector == 0)) || ((kxSector == -1) && (kySector == 0)))
	{
...
long BosonOnSquareLatticeWannierSpace::EvaluatePartialDensityMatrixParticlePartitionCore (int minIndex, int nbrIndex, ParticleOnSphere* complementaryHilbertSpace,  ParticleOnSphere* destinationHilbertSpace,
											   ComplexVector& groundState,  HermitianMatrix* densityMatrix)
{
  BosonOnSquareLatticeWannierSpace* TmpHilbertSpace =  (BosonOnSquareLatticeWannierSpace*) complementaryHilbertSpace;
  BosonOnSquareLatticeWannierSpace* TmpDestinationHilbertSpace =  (BosonOnSquareLatticeWannierSpace*) destinationHilbertSpace;
...
int BosonOnSquareLatticeWannierSpace::FindStateIndexFromArray(int* stateDescription)
{
...
HermitianMatrix BosonOnSquareLatticeWannierSpace::EvaluatePartialDensityMatrixParticlePartition (int nbrParticleSector, int kxSector, int kySector, int nbrGroundStates, ComplexVector* groundStates, double* weights, AbstractArchitecture* architecture)
{
  if (nbrParticleSector == 0)
    {
      if (((kxSector == 0) && (kySector == 0)) || ((kxSector == -1) && (kySector == 0)))
