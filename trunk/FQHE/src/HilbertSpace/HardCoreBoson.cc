...
HardCoreBoson::HardCoreBoson(int nbrBosons, int nbrStates, unsigned long memory)
{
#ifdef __64_BITS__  
  if (nbrStates>64)    
#else
...
#endif
    {
      cout<<"HardCoreBoson: Cannot represent the "<<nbrStates<<" states requested in a single word"<<endl;
      exit(1);
    }
...
HardCoreBoson& HardCoreBoson::operator = (const HardCoreBoson& bosons)
{
  if ((this->HilbertSpaceDimension != 0) && (this->Flag.Shared() == false) && (this->Flag.Used() == true))
    {
      delete[] this->StateDescription;
...
AbstractHilbertSpace* HardCoreBoson::Clone()
{
  return new HardCoreBoson(*this);
}
...
void HardCoreBoson::SetTargetSpace(HardCoreBoson* targetSpace)
{
...
int HardCoreBoson::GetTargetHilbertSpaceDimension()
{
...
int HardCoreBoson::GetParticleStatistic()
{
...
int HardCoreBoson::GetHilbertSpaceAdditionalSymmetry()
{
  return HardCoreBoson::NoSymmetry;
}
...
List<AbstractQuantumNumber*> HardCoreBoson::GetQuantumNumbers ()
{
...
// return value = pointer to corresponding quantum number
AbstractQuantumNumber* HardCoreBoson::GetQuantumNumber (int index)
{
...
AbstractHilbertSpace* HardCoreBoson::ExtractSubspace (AbstractQuantumNumber& q, 
...
int HardCoreBoson::AdAdAA (int index, int m1, int m2, int n1, int n2)
{
...
double HardCoreBoson::AA (int index, int n1, int n2)
{
...
int HardCoreBoson::AdAd (int m1, int m2)
{
...
int HardCoreBoson::AdA (int index, int m, int n)
{
  if (m!=n)
    {
      int StateHighestBit = this->StateHighestBit[index];
...
int HardCoreBoson::AdA (int index, int m)
{
  int StateHighestBit = this->StateHighestBit[index];
  unsigned long State = this->StateDescription[index];
...
Complex HardCoreBoson::EvaluateWaveFunction (RealVector& state, RealVector& position, AbstractFunctionBasis& basis)
{
...
Complex HardCoreBoson::EvaluateWaveFunctionWithTimeCoherence (RealVector& state, RealVector& position, 
...
void HardCoreBoson::InitializeWaveFunctionEvaluation (bool timeCoherence)
{
}
...
ostream& HardCoreBoson::PrintState (ostream& Str, int state)
{
...
int HardCoreBoson::FindStateIndex(unsigned long stateDescription, int highestBit)
{
...
//
int HardCoreBoson::EvaluateHilbertSpaceDimension(int nbrBosons,int nbrStates)
{  
...
//
int HardCoreBoson::GenerateStates(int nbrBosons,int nbrStates)
{
...
void HardCoreBoson::GenerateLookUpTable(unsigned long memory)
{
