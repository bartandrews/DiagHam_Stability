...
#ifndef BOSONONSPHERELONG_H
#define BOSONONSPHERELONG_H
...
class BosonOnSphereLong :  public ParticleOnSphere
{
 ...
 protected:
  ...
  FermionOnSphereLong* FermionBasis;
 ...
 public:
  ...
  BosonOnSphereLong (int nbrBosons, int totalLz, int lzMax);

  // copy constructor (without duplicating datas)
  //
  // bosons = reference on the hilbert space to copy to copy
  BosonOnSphereLong(const BosonOnSphereLong& bosons);

  // destructor
  //
  virtual ~BosonOnSphereLong ();

  // assignement (without duplicating datas)
  //
  // bosons = reference on the hilbert space to copy to copy
  // return value = reference on current hilbert space
  BosonOnSphereLong& operator = (const BosonOnSphereLong& bosons);
...
inline int BosonOnSphereLong::GetParticleStatistic()
{
...
inline ULONGLONG BosonOnSphereLong::BosonToFermion(unsigned long*& initialState, int& initialStateLzMax)
{
...
inline void BosonOnSphereLong::FermionToBoson(ULONGLONG initialState, int initialStateLzMax, unsigned long*& finalState, int& finalStateLzMax)
{
  finalStateLzMax = 0;
  while (initialStateLzMax >= 0)
    {
      ULONGLONG TmpState = (~initialState - 1ul) ^ (~initialState);
      TmpState &= ~(TmpState >> 1);
...
#endif
//      cout << TmpPower << endl;
      finalState[finalStateLzMax] = (ULONGLONG) TmpPower;
      ++TmpPower;
...
inline void BosonOnSphereLong::GetMonomial(long index, unsigned long*& finalState)
{
  int Index = 0;
  ULONGLONG InitialState = this->FermionBasis->StateDescription[index];
  int InitialStateLzMax  = this->FermionBasis->LzMax;
  int TmpLz = InitialStateLzMax  - this->NbrBosons + 1;
  while (InitialStateLzMax >= 0)
    {
      while ((InitialStateLzMax >= 0) && (((InitialState >> InitialStateLzMax) & ((ULONGLONG) 0x1ul)) != ((ULONGLONG) 0x0ul)))
	{
...
inline void BosonOnSphereLong::ConvertToMonomial(unsigned long* initialState, int initialStateLzMax, unsigned long*& finalState)
{
  int Index = 0;
...
inline void BosonOnSphereLong::ConvertToMonomial(ULONGLONG initialState, int initialStateLzMax, unsigned long*& finalState)
{
  int Index = 0;
  int TmpLz = initialStateLzMax - this->NbrBosons + 1;
  while (initialStateLzMax >= 0)
...
inline ULONGLONG BosonOnSphereLong::ConvertFromMonomial(unsigned long* initialState)
{
  ULONGLONG Tmp = ((ULONGLONG) 0x0ul);
  for (int i = 0; i < this->NbrBosons; ++i)
    Tmp |= ((ULONGLONG) 0x1ul) << (initialState[i] + ((unsigned long) (this->NbrBosons - i)) - 1ul);
  return Tmp;
}
