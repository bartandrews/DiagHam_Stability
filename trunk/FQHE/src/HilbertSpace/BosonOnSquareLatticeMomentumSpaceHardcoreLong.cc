...
#include "config.h"
#include "HilbertSpace/BosonOnSquareLatticeMomentumSpaceHardcoreLong.h"
...
BosonOnSquareLatticeMomentumSpaceHardcoreLong::BosonOnSquareLatticeMomentumSpaceHardcoreLong ()
{
}
...
BosonOnSquareLatticeMomentumSpaceHardcoreLong::BosonOnSquareLatticeMomentumSpaceHardcoreLong (int nbrBosons, int nbrSiteX, int nbrSiteY, int kxMomentum, int kyMomentum, unsigned long memory)
{  
...
BosonOnSquareLatticeMomentumSpaceHardcoreLong::BosonOnSquareLatticeMomentumSpaceHardcoreLong(const BosonOnSquareLatticeMomentumSpaceHardcoreLong& bosons)
{
...
BosonOnSquareLatticeMomentumSpaceHardcoreLong& BosonOnSquareLatticeMomentumSpaceHardcoreLong::operator = (const BosonOnSquareLatticeMomentumSpaceHardcoreLong& bosons)
{
  if (bosons.TargetSpace != &bosons)
    this->TargetSpace = bosons.TargetSpace;
  else
...
AbstractHilbertSpace* BosonOnSquareLatticeMomentumSpaceHardcoreLong::Clone()
{
  return new BosonOnSquareLatticeMomentumSpaceHardcoreLong(*this);
}
...
bool BosonOnSquareLatticeMomentumSpaceHardcoreLong::WriteHilbertSpace (char* fileName)
{
  ofstream File;
...
void BosonOnSquareLatticeMomentumSpaceHardcoreLong::GetOccupied(int state, int* orbitals)
{
  ULONGLONG TmpState = this->StateDescription[state];
  int TmpLzMax = this->StateLzMax[state];
  int i = 0;
  for (int l = 0; l <= TmpLzMax; ++l)
    if (((TmpState >> l) & ((ULONGLONG) 0x1ul)) != ((ULONGLONG) 0x0ul))
      orbitals[i++] = l;
}
...
ostream& BosonOnSquareLatticeMomentumSpaceHardcoreLong::PrintState (ostream& Str, int state)
{
  ULONGLONG TmpState = this->StateDescription[state];
  int TmpLzMax = this->StateLzMax[state];
  Str << "[";
...
long BosonOnSquareLatticeMomentumSpaceHardcoreLong::GenerateStates(int nbrBosons, int currentKx, int currentKy, int currentTotalKx, int currentTotalKy, int currentFermionicPosition, long pos)
{
...
long BosonOnSquareLatticeMomentumSpaceHardcoreLong::EvaluateHilbertSpaceDimension(int nbrBosons, int currentKx, int currentKy, int currentTotalKx, int currentTotalKy)
{
  if (nbrBosons < 0)
    return 0l;
  if (currentKy < 0)
...
HermitianMatrix BosonOnSquareLatticeMomentumSpaceHardcoreLong::EvaluatePartialDensityMatrixParticlePartition (int nbrParticleSector, int kxSector, int kySector, ComplexVector& groundState, AbstractArchitecture* architecture)
{
  ...
  BosonOnSquareLatticeMomentumSpaceHardcoreLong SubsytemSpace (nbrParticleSector, this->NbrSiteX, this->NbrSiteY, kxSector, kySector);
  HermitianMatrix TmpDensityMatrix (SubsytemSpace.GetHilbertSpaceDimension(), true);
  BosonOnSquareLatticeMomentumSpaceHardcoreLong ComplementarySpace (ComplementaryNbrParticles, this->NbrSiteX, this->NbrSiteY, ComplementaryKxMomentum, ComplementaryKyMomentum);
...
HermitianMatrix BosonOnSquareLatticeMomentumSpaceHardcoreLong::EvaluatePartialDensityMatrixParticlePartition (int nbrParticleSector, int kxSector, int kySector, 
