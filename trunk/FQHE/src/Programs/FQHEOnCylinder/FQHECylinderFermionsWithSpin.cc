...
int main(int argc, char** argv)
{
  ...
  OptionManager Manager ("FQHECylinderFermionsWithSpin" , "0.01");
  ...
  if (Manager.ProceedOptions(argv, argc, cout) == false)
    {
      cout << "see man page for option syntax or type FQHECylinderFermionsWithSpin -h" << endl;
      return -1;
    }
  ...
  int NbrUp = (NbrFermions + TotalSpin) >> 1;
  int NbrDown = (NbrFermions - TotalSpin) >> 1;
  ...
  if (OneBodyPseudoPotentials[2] == 0)
	...
	{
	  sprintf (OutputName, "fermions_%s%s_%s_n_%d_2s_%d_sz_%d.dat", GeometryName, DiscreteSymmetryName, Manager.GetString("interaction-name"), 
		   NbrFermions, LzMax, TotalSpin);
	}
      else
	{
	  sprintf (OutputName, "fermions_%s%s_%s_n_%d_2s_%d_sz_%d_fluxup_%.6f_fluxdown_%.6f.dat", GeometryName, DiscreteSymmetryName, 
		   Manager.GetString("interaction-name"), 
		   NbrFermions, LzMax, TotalSpin, Ratio, Manager.GetDouble("spinup-flux"), Manager.GetDouble("spindown-flux"));
	}
    }
  else
    {
      if ((Manager.GetDouble("spinup-flux") == 0.0) && (Manager.GetDouble("spindown-flux") == 0.0))
	{
	  sprintf (OutputName, "fermions_%s%s_%s_n_%d_2s_%d.dat", GeometryName, DiscreteSymmetryName, Manager.GetString("interaction-name"), NbrFermions, LzMax);
	}
      else
	{
	  sprintf (OutputName, "fermions_%s%s_%s_n_%d_2s_%d_fluxup_%.6f_fluxdown_%.6f.dat", GeometryName, DiscreteSymmetryName, 
		   Manager.GetString("interaction-name"), NbrFermions, LzMax,
  ...
  for (; L <= Max; L += 2)
      ...
      if (Space->GetHilbertSpaceDimension() > 0)
	{
	  ...
	  Hamiltonian = new ParticleOnCylinderWithSpinGenericHamiltonian(Space, NbrFermions, LzMax, Ratio,
