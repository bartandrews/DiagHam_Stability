...
int main(int argc, char** argv)
{
  ...
  (*SystemGroup) += new SingleDoubleOption ('t',"mixing-angle", "mixing angle between landau levels",0);
  (*SystemGroup) += new SingleIntegerOption ('\n',"mixing-level", "index of LL to mix with LLL",2);
  ...
  double MixingAngle = Manager.GetDouble("mixing-angle");
  ...
  bool FirstRun = true;
  int MixingLevel = Manager.GetInteger("mixing-level");
  
  if(MixingLevel % 2 != 0)
  {
    std::cout << "Only even level mixing is currently supported.\n";
    return -1;
  }

  char* OutputNameLz = new char [256];
  sprintf (OutputNameLz, "bosons_torus_kysym_delta_n_%d_2s_%d_ratio_%f_theta_%f_l_%d.dat", NbrParticles, MaxMomentum, XRatio,MixingAngle,MixingLevel);
  ofstream File;
  ...
  for (; Momentum <= Max; ++Momentum)
	...
	Memory = Architecture.GetArchitecture()->GetLocalMemory();
      ...
      AbstractQHEHamiltonian* Hamiltonian = new ParticleOnTorusDeltaHamiltonian (Space, NbrParticles, MaxMomentum, XRatio, MixingAngle, MixingLevel, Architecture.GetArchitecture(), Memory);
