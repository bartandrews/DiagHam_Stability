...
int main(int argc, char** argv)
{
  ...
  (*SystemGroup) += new SingleIntegerOption  ('k', "ky", "constraint of momentum in y-direction (optional)", 0);
  ...
  if (FQHEOnLatticeFindSystemInfoWithKyFromVectorFileName(VectorFiles[0], NbrBosons, Lx, Ly, Ky, Interaction, NbrFluxQuanta, TmpI, Statistics, HardCore) == false)
    {
  ...
  HardCore=(HardCore||(Manager.GetBoolean("hard-core")&& !KySymmetry));
  if (Manager.GetBoolean("no-hard-core"))
    HardCore=false;
...
// 	    cout << "Potential translation " << 0 << "->"<<fi<<endl;
  ...
  if (KySymmetry)
    {
      cout << "N="<<NbrBosons<<", Lx="<<Lx<<", Ly="<<Ly<<", Q="<<NbrFluxQuanta<<", Ky="<<Ky<<endl;
      Space = new BosonOnLatticeKy(NbrBosons, Lx, Ly, Ky, NbrFluxQuanta, MemorySpace);      
    }
  else
    {
      cout << "N="<<NbrBosons<<", Lx="<<Lx<<", Ly="<<Ly<<", Q="<<NbrFluxQuanta<<endl;
      if (HardCore)
