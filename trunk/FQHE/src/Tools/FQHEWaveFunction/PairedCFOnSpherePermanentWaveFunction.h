...
#ifndef PAIREDCFONSPHEREPERMANENTWAVEFUNCTION_H
#define PAIREDCFONSPHEREPERMANENTWAVEFUNCTION_H
...
class PairedCFOnSpherePermanentWaveFunction: public Abstract1DComplexTrialFunctionOnSphere
{
 ...
 protected:
  ...
  double ElementNorm;
  // factor used to multiply each element of the permanent factor
  double PermanentNorm;

  // permanent part of the wavefunction
  Complex Permanent;
 ...
 public:
  ...
  // default constructor
  //
  PairedCFOnSpherePermanentWaveFunction();
  ...
  PairedCFOnSpherePermanentWaveFunction(int nbrParticles, int nbrLandauLevels, int nbrEffectiveFlux, double MooreReadCoefficient,
			       double * CFCoefficients, bool correctPrefactors=false, int jastrowPower=2);

  // copy constructor
  //
  // function = reference on the wave function to copy
  PairedCFOnSpherePermanentWaveFunction(const PairedCFOnSpherePermanentWaveFunction& function);
...
inline double PairedCFOnSpherePermanentWaveFunction::fsgn(int x)
{
  if (x & 1) 
    return -1.0;
  else 
...
#endif //PAIREDCFONSPHEREPERMANENTWAVEFUNCTION
