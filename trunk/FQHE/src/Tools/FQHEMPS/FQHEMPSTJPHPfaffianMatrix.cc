...
#include "config.h"
#include "Tools/FQHEMPS/FQHEMPSTJPHPfaffianMatrix.h"
...
FQHEMPSTJPHPfaffianMatrix::FQHEMPSTJPHPfaffianMatrix()
{
}
...
FQHEMPSTJPHPfaffianMatrix::FQHEMPSTJPHPfaffianMatrix(int laughlinIndex, int pLevel, int nbrBMatrices, const char* cftDirectory, bool bosonicVersion, bool useRational, 
...
FQHEMPSTJPHPfaffianMatrix::FQHEMPSTJPHPfaffianMatrix(int laughlinIndex, int pLevel, const char* fileName, 
						 bool trimChargeIndices, bool cylinderFlag, double kappa, 
						 bool torusFlag, int nbrFluxQuanta, double aspectRatio, double angle, double fluxInsertion)
{
  this->RIndex = 2;
...
int FQHEMPSTJPHPfaffianMatrix::GetMatrixNaturalNbrParticles(int nbrFluxQuanta, bool padding)
{
...
void FQHEMPSTJPHPfaffianMatrix::CreateBMatrices (const char* cftDirectory, AbstractArchitecture* architecture)
{
...
int FQHEMPSTJPHPfaffianMatrix::ComputeLinearizedIndexArrays()
{
  ...
  this->StartingIndexPerPLevelCFTSectorQValueU1Sectors = new int***** [this->PLevel + 1];
  this->NbrIndexPerPLevelCFTSectorQValueU1Sectors = new int**** [this->PLevel + 1];
  ...
  for (int i = 0; i <= this->PLevel; ++i)
    {
      ...
      this->StartingIndexPerPLevelCFTSectorQValueU1Sectors[i] = new int**** [this->NbrCFTSectors];      
      this->NbrIndexPerPLevelCFTSectorQValueU1Sector[i] = new int** [this->NbrCFTSectors];
      this->NbrIndexPerPLevelCFTSectorQValueU1Sectors[i] = new int*** [this->NbrCFTSectors];      
      for (int l = 0; l < this->NbrCFTSectors; ++l)
	{
	  ...
	  this->NbrIndexPerPLevelCFTSectorQValueU1Sectors[i][l] = new int** [this->NbrNValuesPerPLevelCFTSector[i][l]];

// warning unfinished work : StartingIndexPerPLevelCFTSectorQValueU1Sectors not properly created
	  ...
	  for (int j = this->NInitialValuePerPLevelCFTSector[i][l];  j <= this->NLastValuePerPLevelCFTSector[i][l]; ++j)
	    {
	      ...
	      this->StartingIndexPerPLevelCFTSectorQValueU1Sectors[i][l][j - this->NInitialValuePerPLevelCFTSector[i][l]] = new int** [i + 1];
...
void FQHEMPSTJPHPfaffianMatrix::GetMatrixBoundaryIndices(int& rowIndex, int& columnIndex, bool padding)
{
  int MinQ;
  int MaxQ;
...
void FQHEMPSTJPHPfaffianMatrix::ComputeChargeIndexRange(int pLevel, int cftSector, int& minQ, int& maxQ)
{
...
SparseRealMatrix** FQHEMPSTJPHPfaffianMatrix::GetSphereSiteDependentMatrices(int nbrFluxQuanta)
{
  LongRational CentralCharge12 (this->CentralCharge);
  cout << "central charge = " << CentralCharge12 << endl;
  CentralCharge12 /= 12l;
  ...
  int NValueShift;
  int QValueLgh;
  int QValueDenominatorLgh;
  int QValueMR;
  int QValueDenominatorMR;
  double ExtraCylinderFactor = 1.0;
  ...
  for (int n = 0; n <= this->PLevelShift; ++n)
      ...
      for (int m = 0; m <= this->PLevelShift; ++m)
	{
	  for (int p = 0; p <= this->PLevelShift; ++p)
	    {
	      if (((this->PLevelShift + p - n - m) <= this->PLevel)  && ((this->PLevelShift + p - n - m) >= 0))
		{
		  BosonOnDiskShort* TmpSpaceChargedLgh = U1BosonBasis[p];
		  BosonOnDiskShort* TmpSpaceChargedMR = U1BosonBasis[m];
		  BosonOnDiskShort* TmpSpaceNeutral = U1BosonBasis[n];
		  RealMatrix& TmpOrthogonalBasisIdentityLeft = OrthogonalBasisIdentityLeft[n];
		  RealMatrix& TmpOrthogonalBasisPsiLeft = OrthogonalBasisPsiLeft[n];
		  RealMatrix& TmpOrthogonalBasisIdentityRight = OrthogonalBasisIdentityRight[n];
		  ...
		  RealSymmetricMatrix& TmpScalarProductIdentity = ScalarProductIdentity[n];
		  RealSymmetricMatrix& TmpScalarProductPsi = ScalarProductPsi[n];
		  for (int ChargedIndexMR = 0; ChargedIndexMR < TmpSpaceChargedMR->GetHilbertSpaceDimension(); ++ChargedIndexMR)
		    {	      
		      for (int ChargedIndexLgh = 0; ChargedIndexLgh < TmpSpaceChargedLgh->GetHilbertSpaceDimension(); ++ChargedIndexLgh)
			{	      
			  for (int NMR = this->NInitialValuePerPLevelCFTSector[this->PLevelShift + p - n][0] + 1; NMR <= this->NLastValuePerPLevelCFTSector[this->PLevelShift + p - n][0]; ++NMR)
			    {
			      for (int NLgh = this->NInitialValuePerPLevelCFTSector[this->PLevelShift + p - n][0] + 1; NLgh <= this->NLastValuePerPLevelCFTSector[this->PLevelShift + p - n][0]; ++NLgh)
				{
				  for (int NeutralIndex1 = 0; NeutralIndex1 < TmpOrthogonalBasisIdentityLeft.GetNbrColumn(); ++NeutralIndex1)
				    {
				      for (int NeutralIndex2 = 0; NeutralIndex2 < TmpOrthogonalBasisIdentityLeft.GetNbrColumn(); ++NeutralIndex2)
					{
					  ++TmpNbrElementPerRow[this->Get2RMatrixIndexV2(this->PLevelShift + p - n - m, 0, NLgh - 1, NMR - 1, p, ChargedIndexLgh, m, ChargedIndexMR, NeutralIndex1)];
					}
				    }
				}
			    }
			  for (int NMR = this->NInitialValuePerPLevelCFTSector[this->PLevelShift + p - n][1] + 1; NMR <= this->NLastValuePerPLevelCFTSector[this->PLevelShift + p - n][1]; ++NMR)
			    {
			      for (int NLgh = this->NInitialValuePerPLevelCFTSector[this->PLevelShift + p - n][1] + 1; NLgh <= this->NLastValuePerPLevelCFTSector[this->PLevelShift + p - n][1]; ++NLgh)
				{
				  for (int NeutralIndex1 = 0; NeutralIndex1 < TmpOrthogonalBasisPsiLeft.GetNbrColumn(); ++NeutralIndex1)
				    {
				      for (int NeutralIndex2 = 0; NeutralIndex2 < TmpOrthogonalBasisPsiLeft.GetNbrColumn(); ++NeutralIndex2)
					{
					  ++TmpNbrElementPerRow[this->Get2RMatrixIndexV2(this->PLevelShift + p - n - m, 1, NLgh - 1, NMR - 1, p, ChargedIndexLgh, m, ChargedIndexMR, NeutralIndex1)];
					}
				    }
				}
			    }
			      ...
			      for (int NLgh = this->NInitialValuePerPLevelCFTSector[this->PLevelShift + p - n][0] + 1; NLgh <= this->NLastValuePerPLevelCFTSector[this->PLevelShift + p - n][0]; ++NLgh)
				{
				  for (int NeutralIndex1 = 0; NeutralIndex1 < TmpOrthogonalBasisIdentityLeft.GetNbrColumn(); ++NeutralIndex1)
				      ...
				      for (int NeutralIndex2 = 0; NeutralIndex2 < TmpOrthogonalBasisIdentityLeft.GetNbrColumn(); ++NeutralIndex2)
					{
					  ...
					  TmpB0Matrix.SetMatrixElement(this->Get2RMatrixIndexV2(this->PLevelShift + p - n - m, 0, NLgh - 1, NMR - 1, p, ChargedIndexLgh, m, ChargedIndexMR, NeutralIndex1),
								       this->Get2RMatrixIndexV2(this->PLevelShift + p - n - m, 0, NLgh, NMR, p, ChargedIndexLgh, m, ChargedIndexMR, NeutralIndex2), Tmp);
					}
				    }
				}
			    }
			      ...
			      for (int NLgh = this->NInitialValuePerPLevelCFTSector[this->PLevelShift + p - n][0] + 1; NLgh <= this->NLastValuePerPLevelCFTSector[this->PLevelShift + p - n][0]; ++NLgh)
				{
				  for (int NeutralIndex1 = 0; NeutralIndex1 < TmpOrthogonalBasisPsiLeft.GetNbrColumn(); ++NeutralIndex1)
				      ...
				      for (int NeutralIndex2 = 0; NeutralIndex2 < TmpOrthogonalBasisPsiLeft.GetNbrColumn(); ++NeutralIndex2)
					{
					  ...
					  TmpB0Matrix.SetMatrixElement(this->Get2RMatrixIndexV2(this->PLevelShift + p - n - m, 1, NLgh - 1, NMR - 1, p, ChargedIndexLgh, m, ChargedIndexMR, NeutralIndex1),
								       this->Get2RMatrixIndexV2(this->PLevelShift + p - n - m, 1, NLgh, NMR, p, ChargedIndexLgh, m, ChargedIndexMR, NeutralIndex2), Tmp);
					}
				    }
				}
			    }
  ...
  for (int i = 0; i < MatrixSize; ++i)
    TmpNbrElementPerRow[i] = 0;
  FactorialCoefficient Coef;
  unsigned long* PartitionLgh1 = new unsigned long [this->PLevel + 2];
  unsigned long* PartitionLgh2 = new unsigned long [this->PLevel + 2];
  unsigned long* PartitionMR1 = new unsigned long [this->PLevel + 2];
  unsigned long* PartitionMR2 = new unsigned long [this->PLevel + 2];
  ...
  for (int V0MatrixIndex = 0; V0MatrixIndex < NbrV0MatrixIndices; ++V0MatrixIndex)
	...
	TmpNbrElementPerRow[i] = 0;
      ...
      for (int n = 0; n <= this->PLevelShift; ++n)
	{
	  for (int ChargeSectorLgh1 = 0; ChargeSectorLgh1 <= this->PLevelShift; ++ChargeSectorLgh1)
	    {
	      for (int ChargeSectorMR1 = 0; ChargeSectorMR1 <= this->PLevelShift; ++ChargeSectorMR1)
		{
		  if (((this->PLevelShift + ChargeSectorLgh1 - n - ChargeSectorMR1) <= this->PLevel) && ((this->PLevelShift + ChargeSectorLgh1 - n - ChargeSectorMR1) >= 0))
		    {
		      BosonOnDiskShort* TmpSpaceChargedLgh1 = U1BosonBasis[ChargeSectorLgh1];
		      BosonOnDiskShort* TmpSpaceChargedMR1 = U1BosonBasis[ChargeSectorMR1];
		      BosonOnDiskShort* TmpSpaceNeutral1 = U1BosonBasis[n];
		      RealMatrix& TmpOrthogonalBasisIdentity1 = OrthogonalBasisIdentityLeft[n];
		      RealMatrix& TmpOrthogonalBasisPsi1 = OrthogonalBasisPsiLeft[n];
		      for (int m = 0; m <= this->PLevelShift; ++m)
			{
			  for (int ChargeSectorLgh2 = 0; ChargeSectorLgh2 <= this->PLevelShift; ++ChargeSectorLgh2)
			    {
			      for (int ChargeSectorMR2 = 0; ChargeSectorMR2 <= this->PLevelShift; ++ChargeSectorMR2)
				{
				  if (((this->PLevelShift + ChargeSectorLgh2 - m - ChargeSectorMR2) <= this->PLevel) && ((this->PLevelShift + ChargeSectorLgh2 - m - ChargeSectorMR2) >= 0))
				    {
				      BosonOnDiskShort* TmpSpaceChargedLgh2 = U1BosonBasis[ChargeSectorLgh2];
				      BosonOnDiskShort* TmpSpaceChargedMR2 = U1BosonBasis[ChargeSectorMR2];
				      BosonOnDiskShort* TmpSpaceNeutral2 = U1BosonBasis[m];
				      RealMatrix& TmpOrthogonalBasisIdentity2 = OrthogonalBasisIdentityRight[m];
				      RealMatrix& TmpOrthogonalBasisPsi2 = OrthogonalBasisPsiRight[m];
				      RealMatrix& TmpMatrixPsi01 = MatrixPsi01[n][m];
				      RealMatrix& TmpMatrixPsi10 = MatrixPsi10[n][m];	
				      for (int ChargedIndexLgh1 = 0; ChargedIndexLgh1 < TmpSpaceChargedLgh1->GetHilbertSpaceDimension(); ++ChargedIndexLgh1)
					{	      
					  TmpSpaceChargedLgh1->GetOccupationNumber(ChargedIndexLgh1, PartitionLgh1);
					  for (int ChargedIndexMR1 = 0; ChargedIndexLgh1 < TmpSpaceChargedMR1->GetHilbertSpaceDimension(); ++ChargedIndexMR1)
					    {	      
					      TmpSpaceChargedMR1->GetOccupationNumber(ChargedIndexMR1, PartitionMR1);
					      for (int ChargedIndexLgh2 = 0; ChargedIndexLgh2 < TmpSpaceChargedLgh2->GetHilbertSpaceDimension(); ++ChargedIndexLgh2)
						{	      
						  TmpSpaceChargedLgh2->GetOccupationNumber(ChargedIndexLgh2, PartitionLgh2);
						  for (int ChargedIndexMR2 = 0; ChargedIndexMR2 < TmpSpaceChargedMR2->GetHilbertSpaceDimension(); ++ChargedIndexMR2)
						    {	      
						      TmpSpaceChargedMR2->GetOccupationNumber(ChargedIndexMR2, PartitionMR2);
						      int NLgh1;
						      int NLgh2;
						      int NMR1;
						      int NMR2;
						      NLgh2 = (2 * ((ChargedIndexLgh2 - m) - (ChargedIndexLgh1 - n)) - 2 + NValueShift) / 2;
						      NLgh1 = NLgh2 + QValueLgh;
						      NMR2 = 0;
						      NMR1 = NMR2 + QValueMR;
						      if (((NLgh1 >= this->NInitialValuePerPLevelCFTSector[this->PLevelShift + ChargeSectorLgh1 - n - ChargeSectorMR1][0]) 
							   && (NLgh1 <= this->NLastValuePerPLevelCFTSector[this->PLevelShift + ChargeSectorLgh1 - n - ChargeSectorMR1][0]))
							  && ((NLgh2 >= this->NInitialValuePerPLevelCFTSector[this->PLevelShift + ChargeSectorLgh2 - m - ChargeSectorMR2][1]) 
							      && (NLgh2 <= this->NLastValuePerPLevelCFTSector[this->PLevelShift + ChargeSectorLgh2 - m- ChargeSectorMR2][1]))
							  ...
							  for (int NeutralIndex1 = 0; NeutralIndex1 < TmpOrthogonalBasisIdentity1.GetNbrColumn(); ++NeutralIndex1)
							    {
							      for (int NeutralIndex2 = 0; NeutralIndex2 < TmpOrthogonalBasisPsi2.GetNbrColumn(); ++NeutralIndex2)
								{
								  ++TmpNbrElementPerRow[this->Get2RMatrixIndexV2(this->PLevelShift + ChargeSectorLgh1 - n - ChargeSectorMR1, 0, NLgh1, NMR1, ChargeSectorLgh1, ChargedIndexLgh1, 
														 ChargeSectorMR1, ChargedIndexMR1, NeutralIndex1)];
								  ++TmpTotalNbrElements;
								}
							    }
							  
							}
						      NLgh2 = (2 * ((ChargedIndexLgh2 - m) - (ChargedIndexLgh1 - n)) + NValueShift) / 2;
						      NLgh1 = NLgh2 + QValueLgh;
						      NMR2 = 0;
						      NMR1 =  NMR2 + QValueMR;
						      if (((NLgh1 >= this->NInitialValuePerPLevelCFTSector[this->PLevelShift + ChargeSectorLgh1 - n - ChargeSectorMR1][1]) 
							   && (NLgh1 <= this->NLastValuePerPLevelCFTSector[this->PLevelShift + ChargeSectorLgh1 - n - ChargeSectorMR1][1]))
							  && ((NLgh2 >= this->NInitialValuePerPLevelCFTSector[this->PLevelShift + ChargeSectorLgh2 - m - ChargeSectorMR2][0]) 
							      && (NLgh2 <= this->NLastValuePerPLevelCFTSector[this->PLevelShift + ChargeSectorLgh2 - m- ChargeSectorMR2][0]))
							  ...
							  for (int NeutralIndex1 = 0; NeutralIndex1 < TmpOrthogonalBasisPsi1.GetNbrColumn(); ++NeutralIndex1)
							    {
							      for (int NeutralIndex2 = 0; NeutralIndex2 < TmpOrthogonalBasisIdentity2.GetNbrColumn(); ++NeutralIndex2)
								{
								  ++TmpNbrElementPerRow[this->Get2RMatrixIndexV2(this->PLevelShift + ChargeSectorLgh1 - n - ChargeSectorMR1, 1, NLgh1, NMR1, ChargeSectorLgh1, ChargedIndexLgh1, 
														 ChargeSectorMR1, ChargedIndexMR1, NeutralIndex1)];
								  ++TmpTotalNbrElements;
								}
							    }
						...
						{	      
						  ...
						  for (int ChargedIndexLgh2 = 0; ChargedIndexLgh2 < TmpSpaceChargedLgh2->GetHilbertSpaceDimension(); ++ChargedIndexLgh2)
							...
							{	      
							  ...
							  int NMR1;
							  int NMR2;
							  NLgh2 = (2 * ((ChargeSectorLgh2 - m) - (ChargeSectorLgh1 - n)) - 2 + NValueShift) / 2;
							  NLgh1 = NLgh2 + QValueLgh;
							  NMR2 = 0;
							  NMR1 = NMR2 + QValueMR;
							  if (((NLgh1 >= this->NInitialValuePerPLevelCFTSector[this->PLevelShift + ChargeSectorLgh1 - n - ChargeSectorMR1][0]) 
							      ...
							      for (int NeutralIndex1 = 0; NeutralIndex1 < TmpOrthogonalBasisIdentity1.GetNbrColumn(); ++NeutralIndex1)
								{
								  for (int NeutralIndex2 = 0; NeutralIndex2 < TmpOrthogonalBasisPsi2.GetNbrColumn(); ++NeutralIndex2)
								      ...
								      for (int NeutralIndex3 = 0; NeutralIndex3 < TmpSpaceNeutral1->GetHilbertSpaceDimension(); ++NeutralIndex3)
									{
								      ...
								      Tmp *= CreateLaughlinAMatrixElement(QValueLgh, QValueDenominatorLgh, PartitionLgh1, PartitionLgh2, ChargeSectorLgh1, ChargeSectorLgh2, Coef);
								      Tmp *= CreateLaughlinAMatrixElement(QValueMR, QValueDenominatorMR, PartitionMR1, PartitionMR2, ChargeSectorMR1, ChargeSectorMR2, Coef);
								      Tmp *= TmpProjectionFactor;
								      V0Matrix.SetMatrixElement(this->Get2RMatrixIndexV2(this->PLevelShift + ChargeSectorLgh1 - n - ChargeSectorMR1, 0, NLgh1, NMR1, 
															 ChargeSectorLgh1, ChargedIndexLgh1, ChargeSectorMR1, ChargedIndexMR1, NeutralIndex1),
												this->Get2RMatrixIndexV2(this->PLevelShift + ChargeSectorLgh2 - m - ChargeSectorMR2, 1, NLgh2, NMR2, 
															 ChargeSectorLgh1, ChargedIndexLgh2, ChargeSectorMR2, ChargedIndexMR2, NeutralIndex2), Tmp);
								    }
								  
								}
							    }
							  NLgh2 = (2 * ((ChargeSectorLgh2 - m) - (ChargeSectorLgh1 - n)) + NValueShift) / 2;
							  NLgh1 = NLgh2 + QValueLgh;
							  NMR2 = 0;
							  NMR1 = NMR2 + QValueMR;
							  if (((NLgh1 >= this->NInitialValuePerPLevelCFTSector[this->PLevelShift + ChargeSectorLgh1 - n - ChargeSectorMR1][1]) && 
							      ...
							      for (int NeutralIndex1 = 0; NeutralIndex1 < TmpOrthogonalBasisPsi1.GetNbrColumn(); ++NeutralIndex1)
								{
								  for (int NeutralIndex2 = 0; NeutralIndex2 < TmpOrthogonalBasisIdentity2.GetNbrColumn(); ++NeutralIndex2)
								      ...
								      for (int NeutralIndex3 = 0; NeutralIndex3 < TmpSpaceNeutral1->GetHilbertSpaceDimension(); ++NeutralIndex3)
									{
								      ...
								      Tmp *= CreateLaughlinAMatrixElement(QValueMR, QValueDenominatorMR, PartitionMR1, PartitionMR2, ChargeSectorMR1, ChargeSectorMR2, Coef);
								      Tmp *= TmpProjectionFactor;
								      V0Matrix.SetMatrixElement(this->Get2RMatrixIndexV2(this->PLevelShift + ChargeSectorLgh1 - n - ChargeSectorMR1, 1, NLgh1, NMR1, 
															 ChargeSectorLgh1, ChargedIndexLgh1, ChargeSectorMR1, ChargedIndexMR1, NeutralIndex1),
												this->Get2RMatrixIndexV2(this->PLevelShift + ChargeSectorLgh2 - m - ChargeSectorMR2, 0, NLgh2, NMR2, 
															 ChargeSectorLgh1, ChargedIndexLgh2, ChargeSectorMR2, ChargedIndexMR2, NeutralIndex2), Tmp);
								    }
								}
							    }
