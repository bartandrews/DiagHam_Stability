...
ParticleOnCylinderGaffnianHamiltonian::ParticleOnCylinderGaffnianHamiltonian(ParticleOnSphere* particles, int nbrParticles, int maxMomentum,
...
ParticleOnCylinderGaffnianHamiltonian::~ParticleOnCylinderGaffnianHamiltonian() 
{
...
void ParticleOnCylinderGaffnianHamiltonian::SetHilbertSpace (AbstractHilbertSpace* hilbertSpace)
{
  delete[] this->InteractionFactors;
  if (this->FastMultiplicationFlag == true)
    {
...
void ParticleOnCylinderGaffnianHamiltonian::ShiftHamiltonian (double shift)
{
...
void ParticleOnCylinderGaffnianHamiltonian::EvaluateInteractionFactors()
{
  int Pos = 0;
  int m6;
...
Complex ParticleOnCylinderGaffnianHamiltonian::EvaluateInteractionCoefficient(int m1, int m2, int m3, int m4, int m5, int m6)
{
  ...
  double GaussianExp1, GaussianExp2, A, Ap;
  ...
  if (this->ElectricField == 0)
   {
     ...
     //if ((rtimes3*rtimes3+stimes3*stimes3+rtimes3*stimes3+rptimes3*rptimes3+sptimes3*sptimes3+rptimes3*sptimes3) <= 24)
        Coefficient.Re = (6.0 * A * Ap - 2.0 * A * Ap * (GaussianExp1 + GaussianExp2) + A * Ap * GaussianExp1 * GaussianExp2) * exp(- GaussianExp1 - GaussianExp2);
  ...
  if (this->ElectricField == 0)
   {
     ...
     //int sptimes3= 2.0*m5-m4-m6;
     //if ((rtimes3*rtimes3+stimes3*stimes3+rtimes3*stimes3+rptimes3*rptimes3+sptimes3*sptimes3+rptimes3*sptimes3) <= 24)
        Coefficient.Re = (3.0 - 2.0 * GaussianExp1 - 2.0 * GaussianExp2 + 4.0 * GaussianExp1 * GaussianExp2) * exp(- GaussianExp1 - GaussianExp2);
...
int ParticleOnCylinderGaffnianHamiltonian:: NumberOfPermutations(int n1, int n2, int n3)
{
