...
ParticleOnTorusCoulombWithSpinHamiltonian::~ParticleOnTorusCoulombWithSpinHamiltonian() 
{
...
void ParticleOnTorusCoulombWithSpinHamiltonian::SetHilbertSpace (AbstractHilbertSpace* hilbertSpace)
{
  ...
  this->Particles = (ParticleOnTorusWithSpin*) hilbertSpace;
  this->EvaluateInteractionFactors();
}
...
AbstractHilbertSpace* ParticleOnTorusCoulombWithSpinHamiltonian::GetHilbertSpace ()
{
...
int ParticleOnTorusCoulombWithSpinHamiltonian::GetHilbertSpaceDimension ()
{
...
void ParticleOnTorusCoulombWithSpinHamiltonian::SetMagneticG (double magneticG)
{
  this->MagneticG = 0.5 * magneticG;
}
...
Complex ParticleOnTorusCoulombWithSpinHamiltonian::MatrixElement (RealVector& V1, RealVector& V2) 
{
  double x = 0.0;
...
Complex ParticleOnTorusCoulombWithSpinHamiltonian::MatrixElement (ComplexVector& V1, ComplexVector& V2) 
{
  return Complex();
}
...
{
  int LastComponent = firstComponent + nbrComponent;
  double Shift = ((double) this->NbrParticles) * (this->WignerEnergy - this->MagneticG);
  if (this->FastMultiplicationFlag == false)
      ...
      for (int i = firstComponent; i < LastComponent; ++i)
	{
	  ...
	  for (j = 0; j < TmpNbrInteraction; ++j)
	    if ((TmpIndexArray[j] < 0) || (TmpIndexArray[j] > this->Particles->GetHilbertSpaceDimension()))
...
List<Matrix*> ParticleOnTorusCoulombWithSpinHamiltonian::LeftInteractionOperators()
{
  List<Matrix*> TmpList;
  return TmpList;
}
...
List<Matrix*> ParticleOnTorusCoulombWithSpinHamiltonian::RightInteractionOperators()
{
  List<Matrix*> TmpList;
  return TmpList;
}
...
void ParticleOnTorusCoulombWithSpinHamiltonian::EvaluateInteractionFactors()
{
  int Pos = 0;
  int m4;
...
double ParticleOnTorusCoulombWithSpinHamiltonian::EvaluateWignerCrystalEnergy ()
{
...
double ParticleOnTorusCoulombWithSpinHamiltonian::PartialMisraFunction (double n, double x, double min, double max, int nbrSubdivision)
{
  double Sum = 0.0;
  x *= -1.0;
  --nbrSubdivision;
...
void ParticleOnTorusCoulombWithSpinHamiltonian::EnableFastMultiplication()
{
  int Index;
  double Coefficient;
  int m1;
...
ostream& operator << (ostream& Str, ParticleOnTorusCoulombWithSpinHamiltonian& H) 
{
...
MathematicaOutput& operator << (MathematicaOutput& Str, ParticleOnTorusCoulombWithSpinHamiltonian& H) 
{
  RealVector TmpV2 (H.Particles->GetHilbertSpaceDimension(), true);
  RealVector* TmpV = new RealVector [H.Particles->GetHilbertSpaceDimension()];
  for (int i = 0; i < H.Particles->GetHilbertSpaceDimension(); i++)
