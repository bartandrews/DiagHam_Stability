...
#ifndef PARTICLEONSPHERESQUARETOTALISOSPINOPERATOR_H
#define PARTICLEONSPHERESQUARETOTALISOSPINOPERATOR_H
...
class ParticleOnSphereSquareTotalIsospinOperator : public AbstractOperator
{
 ...
 protected:
  ...
  //  twice the total isospin projection (i.e Iz) value
  int TotalIz;
  ...
  // shift due to the Iz^2 and anticommutator terms
  double Shift;
 ...
 public:
  ...
  ParticleOnSphereSquareTotalIsospinOperator(ParticleOnSphereWithSU4Spin* particle, int lzMax, int nbrParticles, int totalIz);

  // copy constructor
  //
  // oper = reference on the operator to copy
  ParticleOnSphereSquareTotalIsospinOperator(const ParticleOnSphereSquareTotalIsospinOperator& oper);

  // destructor
  //
  ~ParticleOnSphereSquareTotalIsospinOperator();
