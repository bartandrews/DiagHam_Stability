...
#include "Hamiltonian/SpinChainLongRangeHamiltonian.h"
...
SpinChainLongRangeHamiltonian::SpinChainLongRangeHamiltonian(AbstractSpinChain* chain, int nbrSpin, double* j, double* jz, double* hz)
{
  this->Chain = chain;
...
SpinChainLongRangeHamiltonian::~SpinChainLongRangeHamiltonian() 
{
  delete[] this->J;
  delete[] this->Jz;
  delete[] this->HalfJ;
...
void SpinChainLongRangeHamiltonian::SetHilbertSpace (AbstractHilbertSpace* hilbertSpace)
{
  delete[] this->SzSzContributions;
...
AbstractHilbertSpace* SpinChainLongRangeHamiltonian::GetHilbertSpace ()
{
  return this->Chain;
}
...
SpinChainLongRangeHamiltonian& SpinChainLongRangeHamiltonian::SetChain(AbstractSpinChain* chain)
{  
  delete[] this->SzSzContributions;
  this->Chain = chain;
  this->SzSzContributions = new double [this->Chain->GetHilbertSpaceDimension()];
...
int SpinChainLongRangeHamiltonian::GetHilbertSpaceDimension ()
{
...
void SpinChainLongRangeHamiltonian::ShiftHamiltonian (double shift)
{
...
Complex SpinChainLongRangeHamiltonian::MatrixElement (RealVector& V1, RealVector& V2) 
{
  double x = 0.0;
  double coef;
...
Complex SpinChainLongRangeHamiltonian::MatrixElement (ComplexVector& V1, ComplexVector& V2) 
{
  return Complex();
}
...
RealVector& SpinChainLongRangeHamiltonian::LowLevelMultiply(RealVector& vSource, RealVector& vDestination) 
{
...
RealVector& SpinChainLongRangeHamiltonian::LowLevelAddMultiply(RealVector& vSource, RealVector& vDestination)
{
...
ComplexVector& SpinChainLongRangeHamiltonian::LowLevelMultiply(ComplexVector& vSource, ComplexVector& vDestination) 
{
  return vDestination;
}
...
ComplexVector& SpinChainLongRangeHamiltonian::LowLevelAddMultiply(ComplexVector& vSource, ComplexVector& vDestination)
{
  return vDestination;
}
...
List<Matrix*> SpinChainLongRangeHamiltonian::LeftInteractionOperators()
{
  List<Matrix*> TmpList;
  int Dim = this->Chain->GetHilbertSpaceDimension();
...
List<Matrix*> SpinChainLongRangeHamiltonian::RightInteractionOperators()
{
  List<Matrix*> TmpList;
  int Dim = this->Chain->GetHilbertSpaceDimension();
  RealSymmetricMatrix* Sx = new RealSymmetricMatrix (Dim, true);
...
void SpinChainLongRangeHamiltonian::EvaluateDiagonalMatrixElements()
{
  int dim = this->Chain->GetHilbertSpaceDimension();
...
ostream& operator << (ostream& Str, SpinChainLongRangeHamiltonian& H) 
{
...
MathematicaOutput& operator << (MathematicaOutput& Str, SpinChainLongRangeHamiltonian& H) 
{
  RealVector TmpV2 (H.Chain->GetHilbertSpaceDimension(), true);
  RealVector* TmpV = new RealVector [H.Chain->GetHilbertSpaceDimension()];
  for (int i = 0; i < H.Chain->GetHilbertSpaceDimension(); i++)
