...
int main(int argc, char** argv)
{
  ...
  if (Manager.GetBoolean("export-entspectrum") == true)
      ...
      if (Manager.GetString("kept-sites") != 0)
	{
	  ...
	  sprintf(ReducedDensityMatrixOutputFileName, "2dtoriccode_entspectrum_x_%d_y_%d_%s.dat", NbrSitesX, NbrSitesY, Manager.GetString("kept-sites"));
	}
      else
	{
	  ReducedDensityMatrixOutputFileName = new char[512];
	  sprintf(ReducedDensityMatrixOutputFileName, "2dtoriccode_entspectrum_x_%d_y_%d_xa_%d_ya_%d.dat", NbrSitesX, NbrSitesY, NbrSitesXA, NbrSitesYA);
	}
      ofstream File;
