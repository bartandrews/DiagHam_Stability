...
int main(int argc, char** argv)
{
  ...
  if (Manager.GetBoolean("low-memory") == false)
	...
	{
	  gettimeofday (&(StartingTime), 0);
	  unsigned long InitialGroundState = (Manager.GetInteger("gs-parity"));
	  double GroundStateNormalizationCoefficient = 1.0 / sqrt((double) GroundStateDimension);
	  double GroundStateSqrNormalizationCoefficient = 1.0 / ((double) GroundStateDimension);
	  ...
	  for (int i = 0; i < MaximumNumberZTerms; ++i)
	    {
	      ProductZTerm ^= HamiltonianZTermMasks[i];
	    }
	  ...
	  unsigned long TmpConfiguration;
	  long GrayCode = 0l;
	  long ChangedBit;
	  int ChangedBitIndex;      
	  ...
	  unsigned long* RegionBConfigurations = new unsigned long[MaxNbrRegionBConfigurations];
  ...
  if (Manager.GetBoolean("export-entspectrum") == true)
      ...
      if (Manager.GetString("kept-sites") != 0)
	{
	  ...
	  sprintf(ReducedDensityMatrixOutputFileName, "xcube_entspectrum_x_%d_y_%d_z_%d_%s.dat", NbrSitesX, NbrSitesY, NbrSitesZ, Manager.GetString("kept-sites"));
	}
      else
	{
	  ReducedDensityMatrixOutputFileName = new char[512];
	  sprintf(ReducedDensityMatrixOutputFileName, "xcube_entspectrum_x_%d_y_%d_z_%d_xa_%d_ya_%d_za_%d.dat", NbrSitesX, NbrSitesY, NbrSitesZ, NbrSitesXA, NbrSitesYA, NbrSitesZA);
	}
      ofstream File;
