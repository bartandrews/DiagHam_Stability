...
int main(int argc, char** argv)
{
  ...
  OptionManager Manager ("SpinChain4QbitsStabilizer" , "0.01");
  ...
  if (Manager.ProceedOptions(argv, argc, cout) == false)
    {
      cout << "see man page for option syntax or type SpinChain4QbitsStabilizer -h" << endl;
      return -1;
    }
  ...
  if ((SpinValue & 1) == 0)
    {
      sprintf (OutputFileName, "spin_%d_%schain_4qbits_stabilizer_n_%d", (SpinValue / 2), BoundaryName, NbrSpins);
    ...
    {
      sprintf (OutputFileName, "spin_%d_2_%schain_4qbits_stabilizer_n_%d", SpinValue, BoundaryName, NbrSpins);
      if (Manager.GetBoolean("use-mirror") == true)
	{
  ...
  if (Chain->GetHilbertSpaceDimension() > 0)
    {
      ...
      SpinChain4QbitsStabilizerHamiltonian* Hamiltonian = new SpinChain4QbitsStabilizerHamiltonian(Chain, NbrSpins, Manager.GetBoolean("use-periodic"));
...
//   MPSMatrices[0].SetMatrixElement(0 ,0, Phase(M_PI * 0.5) / M_SQRT2);
//   MPSMatrices[0].SetMatrixElement(1 ,0, Phase(M_PI * 0.75) / M_SQRT2);
//   MPSMatrices[0].SetMatrixElement(0 ,1, Phase(-M_PI * 0.25) / M_SQRT2);
//   MPSMatrices[0].SetMatrixElement(1 ,1, Phase(M_PI * 0.0) / M_SQRT2);

//   MPSMatrices[1].SetMatrixElement(0 ,0, Phase(M_PI * 0.0) / M_SQRT2);
//   MPSMatrices[1].SetMatrixElement(1 ,0, Phase(-M_PI * 0.75) / M_SQRT2);
//   MPSMatrices[1].SetMatrixElement(0 ,1, Phase(M_PI * 0.25) / M_SQRT2);
//   MPSMatrices[1].SetMatrixElement(1 ,1, Phase(-M_PI * 0.5) / M_SQRT2);

  MPSMatrices[0].SetMatrixElement(0 ,0, Phase(M_PI * 0.0) / M_SQRT2);
  MPSMatrices[0].SetMatrixElement(1 ,0, Phase(M_PI * 0.25) / M_SQRT2);
  MPSMatrices[0].SetMatrixElement(0 ,1, Phase(-M_PI * 0.75) / M_SQRT2);
  MPSMatrices[0].SetMatrixElement(1 ,1, Phase(-M_PI * 0.5) / M_SQRT2);

  MPSMatrices[1].SetMatrixElement(0 ,0, Phase(M_PI * 0.5) / M_SQRT2);
  MPSMatrices[1].SetMatrixElement(1 ,0, Phase(-M_PI * 0.25) / M_SQRT2);
  MPSMatrices[1].SetMatrixElement(0 ,1, Phase(M_PI * 0.75) / M_SQRT2);
  MPSMatrices[1].SetMatrixElement(1 ,1, Phase(M_PI * 0.0) / M_SQRT2);
