...
#ifndef SPIN1_2CHAINWITHTRANSLATIONSANDINVERSIONSYMMETRYLONG_H
#define SPIN1_2CHAINWITHTRANSLATIONSANDINVERSIONSYMMETRYLONG_H
...
class Spin1_2ChainWithTranslationsAndInversionSymmetryLong : public Spin1_2ChainWithTranslationsAndSzSymmetryLong
{
 ...
 protected: 
  ...
  // inversion table 
  ULONGLONG InversionTable[256];

 public:

  // default constructor
  //
  Spin1_2ChainWithTranslationsAndInversionSymmetryLong ();
  ...
  Spin1_2ChainWithTranslationsAndInversionSymmetryLong (int chainLength, int momentum, int translationStep, int inversionSector, int memorySize, int memorySlice);
  ...
  // inversionSector = inversion symmetry sector (can be either +1 or -1)
  // memorySize = memory size in bytes allowed for look-up table
  Spin1_2ChainWithTranslationsAndInversionSymmetryLong (int chainLength, int momentum, int translationStep, int inversionSector, int sz, int memorySize, int memorySlice);

  // copy constructor (without duplicating datas)
  //
  // chain = reference on chain to copy
  Spin1_2ChainWithTranslationsAndInversionSymmetryLong (const Spin1_2ChainWithTranslationsAndInversionSymmetryLong& chain);
  ...
  Spin1_2ChainWithTranslationsAndInversionSymmetryLong& operator = (const Spin1_2ChainWithTranslationsAndInversionSymmetryLong& chain);
...
inline ULONGLONG Spin1_2ChainWithTranslationsAndInversionSymmetryLong::FindCanonicalForm(ULONGLONG stateDescription, int& nbrTranslations, double& szSymmetrySign)
{
...
inline int Spin1_2ChainWithTranslationsAndInversionSymmetryLong::FindOrbitSize(ULONGLONG stateDescription)
{
  ULONGLONG TmpStateDescription = stateDescription;
  int XSize = 1;
  this->ApplySingleXTranslation(TmpStateDescription);      
...
inline bool Spin1_2ChainWithTranslationsAndInversionSymmetryLong::TestMomentumConstraint(ULONGLONG stateDescription)
{
  ULONGLONG TmpStateDescription = stateDescription;
  int XSize = 1;
  this->ApplySingleXTranslation(TmpStateDescription);   
...
inline void Spin1_2ChainWithTranslationsAndInversionSymmetryLong::ApplyInversionSymmetry(ULONGLONG& stateDescription)
{
  ULONGLONG InitialState = stateDescription & (~((((ULONGLONG) 0x1ul) << this->StateShift) - ((ULONGLONG) 0x1ul)));  
  InitialState <<= this->InversionShift;
#ifdef __128_BIT_LONGLONG__
  ULONGLONG TmpState = this->InversionTable[InitialState & 0xfful] << 120;
  TmpState |= this->InversionTable[(InitialState >> 8) & 0xfful] << 112;
  TmpState |= this->InversionTable[(InitialState >> 16) & 0xfful] << 104;
  TmpState |= this->InversionTable[(InitialState >> 24) & 0xfful] << 96;
  TmpState |= this->InversionTable[(InitialState >> 32) & 0xfful] << 88;
  TmpState |= this->InversionTable[(InitialState >> 40) & 0xfful] << 80;
  TmpState |= this->InversionTable[(InitialState >> 48) & 0xfful] << 72;
  TmpState |= this->InversionTable[(InitialState >> 56) & 0xfful] << 64;
  TmpState |= this->InversionTable[(InitialState >> 64) & 0xfful] << 56;
  TmpState |= this->InversionTable[(InitialState >> 72) & 0xfful] << 48;
  TmpState |= this->InversionTable[(InitialState >> 80) & 0xfful] << 40;
  TmpState |= this->InversionTable[(InitialState >> 88) & 0xfful] << 32;
  TmpState |= this->InversionTable[(InitialState >> 96) & 0xfful] << 24;
  TmpState |= this->InversionTable[(InitialState >> 104) & 0xfful] << 16;
  TmpState |= this->InversionTable[(InitialState >> 112) & 0xfful] << 8;
  TmpState |= this->InversionTable[InitialState >> 120]; 
#else
  ULONGLONG TmpState = this->InversionTable[InitialState & 0xfful] << 56;
  ...
  TmpState |= stateDescription & ((ULONGLONG) 0x1ul);
