...
//                 and the inversion symmetry with up to 128 sites            //
//                                                                            //
...
Spin1_2ChainWithTranslationsAndInversionSymmetryLong::Spin1_2ChainWithTranslationsAndInversionSymmetryLong () 
{
...
Spin1_2ChainWithTranslationsAndInversionSymmetryLong::Spin1_2ChainWithTranslationsAndInversionSymmetryLong (int chainLength, int momentum, int translationStep, int inversionSector, int memorySize, int memorySlice) 
{
  this->Flag.Initialize();
  this->ChainLength = chainLength;
  this->FixedSpinProjectionFlag = false;
  this->ComplementaryStateShift = this->ChainLength - translationStep;
  this->StateMask = (((ULONGLONG) 0x1ul) << translationStep) - ((ULONGLONG) 1ul);
  ...
  this->InversionShift = 64 - ((((this->ChainLength + 1) >> 1)) * this->StateShift);
#else
  ...
  this->SzSymmetryMask = (((ULONGLONG) 0x1ul) << this->ChainLength) - ((ULONGLONG) 0x1ul);
...
Spin1_2ChainWithTranslationsAndInversionSymmetryLong::Spin1_2ChainWithTranslationsAndInversionSymmetryLong (int chainLength, int momentum, int translationStep, int inversionSector, int sz, int memorySize, int memorySlice) 
{
  this->Flag.Initialize();
  this->ChainLength = chainLength;
  this->Sz = sz;
  ...
  long TmpHilbertSpaceDimension2 = this->EvaluateHilbertSpaceDimension(this->ChainLength, this->Sz);
  this->StateDescription = new ULONGLONG [TmpHilbertSpaceDimension2];
  long TmpHilbertSpaceDimension = this->GenerateStates(0l, this->ChainLength - 1, (this->ChainLength + this->Sz) >> 1);
  if (TmpHilbertSpaceDimension2 != TmpHilbertSpaceDimension)
    {
      cout << "Hilbert space dimension mismatch " << TmpHilbertSpaceDimension << " " << TmpHilbertSpaceDimension2 << endl;
    }
  this->LargeHilbertSpaceDimension = 0l;
  ULONGLONG TmpState;
  ULONGLONG TmpState2;
...
Spin1_2ChainWithTranslationsAndInversionSymmetryLong::Spin1_2ChainWithTranslationsAndInversionSymmetryLong (const Spin1_2ChainWithTranslationsAndInversionSymmetryLong& chain) 
{
...
Spin1_2ChainWithTranslationsAndInversionSymmetryLong& Spin1_2ChainWithTranslationsAndInversionSymmetryLong::operator = (const Spin1_2ChainWithTranslationsAndInversionSymmetryLong& chain)
{
...
AbstractHilbertSpace* Spin1_2ChainWithTranslationsAndInversionSymmetryLong::Clone()
{
  return new Spin1_2ChainWithTranslationsAndInversionSymmetryLong (*this);
}

// compute the inversion table
//

void Spin1_2ChainWithTranslationsAndInversionSymmetryLong::ComputeInversionTable()
{
  switch (this->StateShift)
    {
    case 1:
      {
	for (ULONGLONG i = ((ULONGLONG) 0x0ul); i <= ((ULONGLONG) 0xfful); ++i)
	  {
	    this->InversionTable[i]  = (i & ((ULONGLONG) 0x01ul)) << 7;
	    this->InversionTable[i] |= (i & ((ULONGLONG) 0x02ul)) << 5;
	    this->InversionTable[i] |= (i & ((ULONGLONG) 0x04ul)) << 3;
	    this->InversionTable[i] |= (i & ((ULONGLONG) 0x08ul)) << 1;
	    this->InversionTable[i] |= (i & ((ULONGLONG) 0x10ul)) >> 1;
	    this->InversionTable[i] |= (i & ((ULONGLONG) 0x20ul)) >> 3;
	    this->InversionTable[i] |= (i & ((ULONGLONG) 0x40ul)) >> 5;
	    this->InversionTable[i] |= (i & ((ULONGLONG) 0x80ul)) >> 7;
	  }
      }
      break;
    case 2:
	...
	for (ULONGLONG i = ((ULONGLONG) 0x0ul); i <= ((ULONGLONG) 0xfful); ++i)
	  {
	    this->InversionTable[i]  = (i & ((ULONGLONG) 0x03ul)) << 6;
	    this->InversionTable[i] |= (i & ((ULONGLONG) 0x0cul)) << 2;
	    this->InversionTable[i] |= (i & ((ULONGLONG) 0x30ul)) >> 2;
	    this->InversionTable[i] |= (i & ((ULONGLONG) 0xc0ul)) >> 6;
	  }
      }
      break;
    case 4:
	...
	for (ULONGLONG i = ((ULONGLONG) 0x0ul); i <= ((ULONGLONG) 0xfful); ++i)
	  {
	    this->InversionTable[i]  = (i & ((ULONGLONG) 0x0ful)) << 4;
	    this->InversionTable[i]  = (i & ((ULONGLONG) 0xf0ul)) >> 4;
	  }
      }
      break;
    default:
      {
	cout << "warning, Spin1_2ChainWithTranslationsAndInversionSymmetryLong does not support a block size of " << this->StateShift << endl;
	for (ULONGLONG i = ((ULONGLONG) 0x0ul); i <= ((ULONGLONG) 0xfful); ++i)
	  {
	    this->InversionTable[i]  = ((ULONGLONG) 0x0ul);
	  }	
      }
    }
}
...
RealMatrix Spin1_2ChainWithTranslationsAndInversionSymmetryLong::EvaluatePartialEntanglementMatrix (int nbrSites, int szSector, RealVector& groundState, AbstractArchitecture* architecture)
{
  if (nbrSites == 0)
    {
      if (szSector == 0)
  ...
  Spin1_2ChainLong TmpDestinationHilbertSpace(nbrSites, szSector, 1000000);
  Spin1_2ChainLong TmpHilbertSpace(this->ChainLength - nbrSites, this->Sz - szSector, 1000000);
