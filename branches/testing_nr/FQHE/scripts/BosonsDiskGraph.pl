...
if (!(defined($ARGV[0])))
  {
    die "usage: BosonsDiskGraph nbr_bosons [xfig,eps,ps] [print_flag]\n";
  }
my $NbrBosons = $ARGV[0];
my $XFigFlag = 0;
...
foreach $TmpFile (<*>)
  {
    if ($TmpFile =~ /bosons\_disk\_.*\_n\_$NbrBosons.*\.dat/ )
      {
...
#    $Max = 10.0;
    ...
    print OUTFILE ("set xrange [0:".$MaxL."]
set yrange [".$Min.":".$Max."]
set xlabel \"Total angular momentum L\"
set ylabel \"E(L)\"\n");
    if ($XFigFlag >= 1)
      {
