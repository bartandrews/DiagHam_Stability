...
FermionOnSphereWithSU3SpinZ3Symmetry::FermionOnSphereWithSU3SpinZ3Symmetry()
{
}
...
FermionOnSphereWithSU3SpinZ3Symmetry::FermionOnSphereWithSU3SpinZ3Symmetry (int nbrFermions, int totalLz, int lzMax, int totalTz, unsigned long memory)
{
  ...
  if (TmpHilbertSpaceDimension != this->HilbertSpaceDimension)
    {
      ...
      cout << "Mismatch in State-count and State Generation in FermionOnSphereWithSU3SpinZ3Symmetry!" << endl;
      exit(1);
    }
...
FermionOnSphereWithSU3SpinZ3Symmetry::FermionOnSphereWithSU3SpinZ3Symmetry(const FermionOnSphereWithSU3SpinZ3Symmetry& fermions)
{
...
FermionOnSphereWithSU3SpinZ3Symmetry::FermionOnSphereWithSU3SpinZ3Symmetry (char* fileName, unsigned long memory)
{
...
FermionOnSphereWithSU3SpinZ3Symmetry& FermionOnSphereWithSU3SpinZ3Symmetry::operator = (const FermionOnSphereWithSU3SpinZ3Symmetry& fermions)
{
  if ((this->HilbertSpaceDimension != 0) && (this->Flag.Shared() == false) && (this->Flag.Used() == true))
    {
      delete[] this->StateDescription;
...
AbstractHilbertSpace* FermionOnSphereWithSU3SpinZ3Symmetry::Clone()
{
  return new FermionOnSphereWithSU3SpinZ3Symmetry(*this);
}
...
RealVector FermionOnSphereWithSU3SpinZ3Symmetry::ConvertToNbodyBasis(RealVector& state, FermionOnSphereWithSU3Spin& nbodyBasis)
{
  ...
  for (int i = 0; i < nbodyBasis.GetHilbertSpaceDimension(); ++i)
	...
	--NewLzMax;	 
      if ((Signature & FERMION_SPHERE_SU3_Z3_SYMMETRIC_BIT) != 0x0ul)
	{
      ...
      else
	{
	  ...
	  if (TmpState != TmpState2)
	    ...
	    TmpVector[i] = TmpCoefficient * state[this->FindStateIndex(TmpState, NewLzMax)] * MSQRT1_3;
	}
    }
  return TmpVector;  
}
...
RealVector FermionOnSphereWithSU3SpinZ3Symmetry::ConvertToSymmetricNbodyBasis(RealVector& state, FermionOnSphereWithSU3Spin& nbodyBasis)
{
  ...
  for (int i = 0; i < nbodyBasis.GetHilbertSpaceDimension(); ++i)
      ...
      else
	{
	  ...
	  if (TmpState != TmpState2)
	    ...
	    TmpVector[this->FindStateIndex(TmpState, NewLzMax)] = TmpCoefficient * state[i] * MSQRT1_3;
	}
    }
  return TmpVector;  
}
