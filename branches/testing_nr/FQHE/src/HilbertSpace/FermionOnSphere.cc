...
RealVector FermionOnSphere::FuseStates (RealVector& leftVector, RealVector& rightVector, int padding, 
					ParticleOnSphere* leftSpace, ParticleOnSphere* rightSpace)
{
  ...
  double Sign = 1.0;
  if (((LeftSpace->NbrFermions * RightSpace->NbrFermions) & 1) == 1)
    Sign *= -1.0;
  double* LeftCoefficients = new double [LeftSpace->LzMax + 1];
  double* RightCoefficients = new double [RightSpace->LzMax + 1];
  double* Coefficients = new double [this->LzMax + 1];
  BinomialCoefficients TmpBinomials (this->LzMax);
  double TmpCoef = ((double) (LeftSpace->LzMax + 1)) / ((double) (this->LzMax + 1));
  for(int i = 0; i <= LeftSpace->LzMax; ++i)
    LeftCoefficients[i] = sqrt(TmpCoef * TmpBinomials.GetNumericalCoefficient(LeftSpace->LzMax, i) / 
			       TmpBinomials.GetNumericalCoefficient(this->LzMax, i + StateShift));
  TmpCoef = ((double) (RightSpace->LzMax + 1)) / ((double) (this->LzMax + 1));
  for(int i = 0; i <= RightSpace->LzMax; ++i)
    RightCoefficients[i] = sqrt(TmpCoef * TmpBinomials.GetNumericalCoefficient(RightSpace->LzMax, i) / 
			       TmpBinomials.GetNumericalCoefficient(this->LzMax, i));

  for (int i = 0; i <  LeftSpace->HilbertSpaceDimension; ++i)
    {
      unsigned long TmpState1 = LeftSpace->StateDescription[i] << StateShift;
      double Coefficient = leftVector[i] * Sign;
      for (int k = 0; k <= LeftSpace->LzMax; ++k)
	if ((TmpState1 & (0x1u << k)) != 0x0ul)
	  Coefficient *= LeftCoefficients[k];
      int TmpLzMax = this->LzMax;
      while ((TmpState1 >> TmpLzMax) == 0x0ul)
	--TmpLzMax;
      for (int j = 0; j < RightSpace->HilbertSpaceDimension; ++j)
	{
	  unsigned long TmpState2 = RightSpace->StateDescription[j];
	  unsigned long TmpState3 = TmpState2 | TmpState1;
	  int TmpIndex = this->FindStateIndex(TmpState3, TmpLzMax);
	  double Coefficient2 = Coefficient;
	  for (int k = 0; k <= RightSpace->LzMax; ++k)
	    if ((TmpState2 & (0x1u << k)) != 0x0ul)
	      Coefficient *= RightCoefficients[k];
	  TmpVector[TmpIndex] = Coefficient2 * rightVector[j];
      }
    }
  delete[] LeftCoefficients;
  delete[] RightCoefficients;
  delete[] Coefficients;
  return TmpVector;
}
