...
//                                                                            //
//                                                                            //
//              class of periodic sand glass spin chain hamiltonian           //
//                                                                            //
...
#include "Hamiltonian/PeriodicSandGlassSpinChainHamiltonian.h"
...
PeriodicSandGlassSpinChainHamiltonian::PeriodicSandGlassSpinChainHamiltonian(AbstractSpinChain* chain, int nbrSandGlass, double jInternal, double jExternal)
{
  this->Chain = chain;
...
PeriodicSandGlassSpinChainHamiltonian::~PeriodicSandGlassSpinChainHamiltonian() 
{
...
void PeriodicSandGlassSpinChainHamiltonian::SetHilbertSpace (AbstractHilbertSpace* hilbertSpace)
{
  delete[] this->SzSzContributions;
...
AbstractHilbertSpace* PeriodicSandGlassSpinChainHamiltonian::GetHilbertSpace ()
{
  return this->Chain;
}
...
PeriodicSandGlassSpinChainHamiltonian& PeriodicSandGlassSpinChainHamiltonian::SetChain(AbstractSpinChain* chain)
{  
  delete[] this->SzSzContributions;
  this->Chain = chain;
  this->SzSzContributions = new double [this->Chain->GetHilbertSpaceDimension()];
...
int PeriodicSandGlassSpinChainHamiltonian::GetHilbertSpaceDimension ()
{
...
void PeriodicSandGlassSpinChainHamiltonian::ShiftHamiltonian (double shift)
{
...
Complex PeriodicSandGlassSpinChainHamiltonian::MatrixElement (RealVector& V1, RealVector& V2) 
{
  double x = 0.0;
  double coef;
  int pos;
  ...
  for (int i = 0; i < dim; i++)
      ...
      if (pos != dim)
	{
      ...
      pos = this->Chain->SmiSpj(TotalIndex + 2, 0, i, coef);
      if (pos != dim)
	{
	  x += V1[pos] * this->HalfJExternal * coef * V2[i];
	}
      pos = this->Chain->SmiSpj(0, TotalIndex + 2, i, coef);
      if (pos != dim)
	{
	  x += V1[pos] * this->HalfJExternal * coef * V2[i];
	}
      pos = this->Chain->SmiSpj(TotalIndex + 4, 3, i, coef);
      if (pos != dim)
	{
	  x += V1[pos] * this->HalfJExternal * coef * V2[i];
	}
      pos = this->Chain->SmiSpj(3, TotalIndex + 4, i, coef);
      if (pos != dim)
	{
	  x += V1[pos] * this->HalfJExternal * coef * V2[i];
	}
...
Complex PeriodicSandGlassSpinChainHamiltonian::MatrixElement (ComplexVector& V1, ComplexVector& V2) 
{
  return Complex();
}
...
RealVector& PeriodicSandGlassSpinChainHamiltonian::LowLevelMultiply(RealVector& vSource, RealVector& vDestination) 
{
...
RealVector& PeriodicSandGlassSpinChainHamiltonian::LowLevelAddMultiply(RealVector& vSource, RealVector& vDestination)
{
...
ComplexVector& PeriodicSandGlassSpinChainHamiltonian::LowLevelMultiply(ComplexVector& vSource, ComplexVector& vDestination) 
{
  return vDestination;
}
...
ComplexVector& PeriodicSandGlassSpinChainHamiltonian::LowLevelAddMultiply(ComplexVector& vSource, ComplexVector& vDestination)
{
  return vDestination;
}
...
List<Matrix*> PeriodicSandGlassSpinChainHamiltonian::LeftInteractionOperators()
{
  List<Matrix*> TmpList;
  return TmpList;
}
...
List<Matrix*> PeriodicSandGlassSpinChainHamiltonian::RightInteractionOperators()
{
  List<Matrix*> TmpList;
  return TmpList;
}
...
void PeriodicSandGlassSpinChainHamiltonian::EvaluateDiagonalMatrixElements()
{
  int dim = this->Chain->GetHilbertSpaceDimension();
  int TotalIndex = 0;
  int ReducedNbrSandGlass = this->NbrSandGlass - 1;
  for (int i = 0; i < dim; i++)
      ...
      for (int j = 0; j < ReducedNbrSandGlass; j++)
	{
      ...
      this->SzSzContributions[i] += this->JInternal * this->Chain->SziSzj(TotalIndex + 3, TotalIndex + 4, i);
      this->SzSzContributions[i] += this->JInternal * this->Chain->SziSzj(TotalIndex + 4, TotalIndex + 1, i);
      this->SzSzContributions[i] += this->JExternal * this->Chain->SziSzj(TotalIndex + 2, 0, i);
      this->SzSzContributions[i] += this->JExternal * this->Chain->SziSzj(TotalIndex + 4, 3, i);      
    }
}
...
ostream& operator << (ostream& Str, PeriodicSandGlassSpinChainHamiltonian& H) 
{
...
MathematicaOutput& operator << (MathematicaOutput& Str, PeriodicSandGlassSpinChainHamiltonian& H) 
{
  RealVector TmpV2 (H.Chain->GetHilbertSpaceDimension(), true);
  RealVector* TmpV = new RealVector [H.Chain->GetHilbertSpaceDimension()];
  for (int i = 0; i < H.Chain->GetHilbertSpaceDimension(); i++)
