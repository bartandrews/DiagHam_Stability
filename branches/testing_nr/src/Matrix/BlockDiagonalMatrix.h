...
class BlockDiagonalMatrix : public Matrix
{
 ...
 private:
  ...
  double Dummy;
  int* BlockRowPosition;
  int* BlockColumnPosition;
  GarbageFlag Flag;
 ...
 public:
  ...
  BlockDiagonalMatrix(int dimension, bool zero = false);

  // constructor from matrix elements (without duplicating datas)
  //
  // blocks =  reference on list of pointers to matirx to use as block
  BlockDiagonalMatrix(List<Matrix*>& blocks);
  ...
  BlockDiagonalMatrix(List<Matrix*>& blocks, int* rowPosition, int* columnPosition) ;

  // copy constructor (without duplicating datas)
  //
  // M = matrix to copy
  BlockDiagonalMatrix(const BlockDiagonalMatrix& M);
  ...
  BlockDiagonalMatrix& operator = (const BlockDiagonalMatrix& M);
  ...
  BlockDiagonalMatrix& operator += (Matrix* M);
 ...
 // add two matrices
  ...
  friend BlockDiagonalMatrix operator + (const BlockDiagonalMatrix& M1, 
					 const BlockDiagonalMatrix& M2);
  ...
  friend BlockDiagonalMatrix operator * (const BlockDiagonalMatrix& M, double x);
  ...
  // x = real number to use
  // return value = product result
  friend BlockDiagonalMatrix operator * (double x, const BlockDiagonalMatrix& M);
  ...
  // x = real number to use
  // return value = reference on current matrix
  BlockDiagonalMatrix& operator *= (double x);
