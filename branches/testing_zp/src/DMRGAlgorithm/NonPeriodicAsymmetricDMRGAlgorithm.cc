...
//                                                                            //
//                                                                            //
//               class of non periodic asymmetric DMRG algorithm              //
//                                                                            //
...
NonPeriodicAsymmetricDMRGAlgorithm::NonPeriodicAsymmetricDMRGAlgorithm (AbstractHamiltonian* blockHamiltonian, 
									...
									int hilbertSpaceSize)
{
  ...
  TwoSpaceTensor TmpInteraction = this->RightLeftInteraction->Interaction(TmpRightInteraction, TmpLeftInteraction);//, TmpLeftInteraction);// , TmpRightInteraction);
...
/*  RealTriDiagonalSymmetricMatrix TmpTest(TmpInteraction.ElementaryMatrix->GetNbrRow());
  ((RealSymmetricMatrix*) (TmpInteraction.ElementaryMatrix))->Householder(TmpTest, 1e-14);
  TmpTest.Diagonalize();
  TmpTest.SortMatrixUpOrder();
  for (int i = 0; i < TmpInteraction.ElementaryMatrix->GetNbrRow(); i++)
    cout << TmpTest.DiagonalElement(i) << " ";
  cout << endl;*/
...
NonPeriodicAsymmetricDMRGAlgorithm::~NonPeriodicAsymmetricDMRGAlgorithm () 
{
}
...
void NonPeriodicAsymmetricDMRGAlgorithm::Constraint(AbstractQuantumNumber* quantumNumber)
{
...
void NonPeriodicAsymmetricDMRGAlgorithm::RunDMRG(int currentBlockIndex) 
{
  ...
  List<ExplicitHamiltonian*> RightHamiltonians;
  ...
  for (int i = 0; i < NbrSubspace; i++)
      ...
      while ((TmpInteraction = IterInteractionOperators()))
	{
      ...
      // transform boundary operators for total right block
      List<Matrix*> InteractionOperators3;
      List<Matrix*> InteractionOperators4;
      TmpInteractionOperators = (this->Blocks[currentBlockIndex]->Hamiltonian->
				 LeftInteractionOperators()); 
      IterInteractionOperators.DefineList(TmpInteractionOperators);
      while ((TmpInteraction = IterInteractionOperators()))
	{
	  InteractionOperators3 += (*TmpInteraction)->Project(Space1SubspaceDescription);
	}
      TmpInteractionOperators = (this->ExplicitInteractionBlockHamiltonian->
				 RightInteractionOperators());
      IterInteractionOperators.DefineList(TmpInteractionOperators);
      while ((TmpInteraction = IterInteractionOperators()))
	{
	  InteractionOperators4 += (*TmpInteraction)->Project(Space2SubspaceDescription);
	}
	...
	Interaction(InteractionOperators1, InteractionOperators2, 
      ...
      // evaluate interaction part of the total right block
      this->RightInteraction->SetTensorProductStructure(FineStructure);
      this->RightInteraction->SetLeftSpaceIndex(0);
      this->RightInteraction->SetRightSpaceIndex(1);
      TwoSpaceTensor* Tensor2Right = (TwoSpaceTensor*) this->RightInteraction->
	Interaction(InteractionOperators3, InteractionOperators4, 
		    Space1Decomposition, Space2Decomposition).Clone();
      ...
      // add hamiltonian contributions of initial right block and added block
      Tensor2Right->AddToFirstSpace(*(RealDiagonalMatrix*)(TmpBlockHamiltonian), 
				    Space1Decomposition, Space2Decomposition);
      Tensor2Right->AddToSecondSpace(*(RealDiagonalMatrix*)(TmpInteractionHamiltonian), 
				     Space1Decomposition, Space2Decomposition);
...
//      TestDiagonalize(Tensor2Right);
//      cout << endl;
//      TestDiagonalize(Tensor2Left);
      LeftHamiltonians += new ExplicitHamiltonian (TmpHilbertSpace, Tensor2Left, InteractionOperators1, InteractionOperators2);
      RightHamiltonians += new ExplicitHamiltonian (TmpHilbertSpace, Tensor2Right, InteractionOperators3, InteractionOperators4);
    }
...
//  return;
  ...
  //
  Lim = (LeftHamiltonians.GetNbrElement() * RightHamiltonians.GetNbrElement());
...
//  return;
  ...
  for (int i = 0; i < NbrSubspace; i++)
      ...
      for (int j = 0; j < NbrDirectSumSubspace[i]; j++)
	{
	  ...
	  RightSpaceFineStructureArray[j] = (FullTensorProductStructure*) 
	    ((TwoSpaceTensor*) (RightHamiltonians[DirectSumSubspaces[i][2 * j + 1]]->
				GetHamiltonian()))->GetTensorProductStructure();
	  RightSpaceSubspaces += SubspaceSpaceConverter(TotalLeftSpaceDecomposition.GetSpaceDimension(), 
							RightHamiltonians[DirectSumSubspaces[i][2 * j + 1]]->
							GetHilbertSpaceDimension(),
	  ...
	  LeftSpaceSubspaces += SubspaceSpaceConverter(TotalLeftSpaceDecomposition.GetSpaceDimension(), 
						       ...
						       GetSubspacePosition(DirectSumSubspaces[i]
									   [2 * j]));
	  RightHamiltonianRepresentationList += ((TwoSpaceTensor*) (RightHamiltonians
	  ...
	  if (j < (NbrDirectSumSubspace[i] - 1))
		...
		RightHamiltonians[DirectSumSubspaces[i][2 * j + 1]]->GetHilbertSpaceDimension();
	  ...
	  TmpStructure.SetDimension(1, RightHamiltonians[DirectSumSubspaces[i][2 * j + 1]]->
				    GetHamiltonian()->GetNbrRow());
      ...
      for (int j = 0; j < NbrDirectSumSubspace[i]; j++)
	{
	  if (DensityMatrixKeptEigenvalues[j] > 0)
	    {		  
	      ...
	      ListLeftQuantumNumber += RightHamiltonians[DirectSumSubspaces[i][2 * j + 1]]->
		GetHilbertSpace()->GetQuantumNumber(0);
	      Matrix* TmpHamiltonian = 
		((RealSymmetricMatrix*) ((TwoSpaceTensor*) RightHamiltonians[DirectSumSubspaces[i][2 * j + 1]]->
	...
	this->TransformOperator(TmpListInteraction, true, TransformationMatrix, 
				...
				this->ExplicitInteractionBlockHamiltonian->
				GetHilbertSpaceDimension());
      TmpListInteraction = this->Blocks[currentBlockIndex]->Hamiltonian->RightInteractionOperators();
	...
	this->TransformOperator(TmpListInteraction, false, TransformationMatrix, 
				...
				this->ExplicitInteractionBlockHamiltonian->
				GetHilbertSpaceDimension());
      cout << "end transfornation left block" << endl;      
  ...
  IterHamiltonians.DefineList(RightHamiltonians);
  while ((TmpHamiltonian = IterHamiltonians()))
    delete *TmpHamiltonian;
  cout << "end third free" << endl;      
  for (int i = 0; i < Lim; i++)
    if (SumQ[i] != 0)
      delete SumQ[i];
  delete[] SumQ;
...
List<Matrix*> NonPeriodicAsymmetricDMRGAlgorithm::TransformOperator(List<Matrix*>& observables, bool spaceFlag, 
...
ExplicitHamiltonian* NonPeriodicAsymmetricDMRGAlgorithm::ReduceHamiltonian (AbstractHamiltonian* hamiltonian, 
									    int truncatedSize) 
{
