...
AbstractQHEOnSphereQuantumWellHamiltonian::~AbstractQHEOnSphereQuantumWellHamiltonian()
{
...
//********************************     M I X E D     T E R M S ***********************************************
//*********************************************************************************************************
      delete[] this->NbrMixedIntraSectorIndicesPerSum;
      for (int i = 0; i < this->NbrMixedIntraSectorSums; ++i)
	{
	  delete[] this->MixedIntraSectorIndicesPerSum[i];
	  delete[] this->InteractionFactorsmixedintra[i];
	}
...
void AbstractQHEOnSphereQuantumWellHamiltonian::AddS2 (int totalLz, int totalSz, double factor, long memory)
{
...
void AbstractQHEOnSphereQuantumWellHamiltonian::AddL2 (int totalLz, int totalSz, double factor, long memory)
{
...
RealVector& AbstractQHEOnSphereQuantumWellHamiltonian::LowLevelAddMultiply(RealVector& vSource, RealVector& vDestination, 
...
RealVector* AbstractQHEOnSphereQuantumWellHamiltonian::LowLevelMultipleAddMultiply(RealVector* vSources, RealVector* vDestinations, int nbrVectors, int firstComponent, int nbrComponent)
{
  int LastComponent = firstComponent + nbrComponent;
  double Coefficient;
  if (this->FastMultiplicationFlag == false)
...
RealVector* AbstractQHEOnSphereQuantumWellHamiltonian::LowLevelMultipleAddMultiplyPartialFastMultiply(RealVector* vSources, RealVector* vDestinations, int nbrVectors, int firstComponent, int nbrComponent)
{
  int LastComponent = firstComponent + nbrComponent;
  ParticleOnSphereWithSpin* TmpParticles = (ParticleOnSphereWithSpin*) this->Particles->Clone();
  int* TmpIndexArray;
...
long AbstractQHEOnSphereQuantumWellHamiltonian::PartialFastMultiplicationMemory(int firstComponent, int lastComponent)
{
  int Index;
...
//*********************************************************************************************************
	  ...
	  for (int j = 0; j < this->NbrMixedInterSectorSums; ++j)
	      ...
	      for (int i1 = 0; i1 < Lim; i1 += 2)
		{
		  ...
		  Coefficient2 = TmpParticles->AuAd(i, TmpIndices[i1 + 1], TmpIndices[i1]);
		  if (Coefficient2 != 0.0)
		    {
		      for (int i2 = 0; i2 < Lim; i2 += 2)
			{
...
void AbstractQHEOnSphereQuantumWellHamiltonian::EnableFastMultiplication()
{
  long MinIndex;
  long MaxIndex;
      ...
      if (this->OneBodyInteractionFactorsupdown != 0)
	{
	  ...
	  for (int j = 0; j <= this->LzMax; ++j)
	      ...
	      if (Index < Dim)
		{
		  ...
		  TmpCoefficientArray[Pos] = Coefficient * OneBodyInteractionFactorsdownup[j];
		  ++Pos;
		}
...
void AbstractQHEOnSphereQuantumWellHamiltonian::PartialEnableFastMultiplication(int firstComponent, int lastComponent)
{
}
...
void AbstractQHEOnSphereQuantumWellHamiltonian::EnableFastMultiplicationWithDiskStorage(char* fileName)
{
  if (this->FastMultiplicationStep == 1)
    {
