...
//*	  - c_{m,down}^+c_{m,down}) 	
//*  -Delta_rho |sum_{m}(c_{m,up}^+c_{m,down} 			     	
//*	  + c_{m,down}^+c_{m,up})
...
int main(int argc, char** argv)
{
  ...
  (*SystemGroup) += new SingleDoubleOption  ('b', "imbalance", "density imbalance", 0.0);
  ...
  double DensityImbalance = Manager.GetDouble("imbalance"); 
  ...
  if (DensityImbalance!=0.0)
    {
      ...
      for (int i=0; i<=LzMax; ++i)
		...
		OneBodyPotentialDownUp[i] = - DensityImbalance;
         }
    }


  char* OutputNameLz = new char [512 + strlen(((SingleStringOption*) Manager["interaction-name"])->GetString())];
  sprintf (OutputNameLz, "fermions_sphere_su2_%s_n_%d_2s_%d_t_%f_b_%f_lz.dat", Manager.GetString("interaction-name"), 
	   NbrFermions, LzMax, DeltaSAS, DensityImbalance);
  ...
  for (; L <= Max; L += 2)
	...
	//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      ...
      Hamiltonian = new ParticleOnSphereQuantumWellHamiltonian(Space, NbrFermions, LzMax, PseudoPotentials,
								   ...
								   OneBodyPotentialUpDown, OneBodyPotentialDownUp,
