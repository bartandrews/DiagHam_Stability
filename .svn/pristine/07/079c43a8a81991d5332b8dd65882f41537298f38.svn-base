...
RealVector& ParticleOnSphereCoulombHamiltonian::LowLevelMultiply(RealVector& vSource, RealVector& vDestination) 
{
...
RealVector& ParticleOnSphereCoulombHamiltonian::LowLevelAddMultiply(RealVector& vSource, RealVector& vDestination)
{
								   ...
								   int firstComponent, int nbrComponent)
{
  int LastComponent = firstComponent + nbrComponent;
  int Dim = this->Particles->GetHilbertSpaceDimension();
  double Shift = -0.5 * ((double) (this->NbrParticles * this->NbrParticles)) / sqrt (0.5 * ((double) this->LzMax));
  double Coefficient;
...
ComplexVector& ParticleOnSphereCoulombHamiltonian::LowLevelMultiply(ComplexVector& vSource, ComplexVector& vDestination) 
{
  return vDestination;
}
...
ComplexVector& ParticleOnSphereCoulombHamiltonian::LowLevelAddMultiply(ComplexVector& vSource, ComplexVector& vDestination)
{
  return vDestination;
}
...
void ParticleOnSphereCoulombHamiltonian::EnableFastMultiplication()
{
  int Index;
  double Coefficient;
  int m1;
...
void ParticleOnSphereCoulombHamiltonian::PartialEnableFastMultiplication(int firstComponent, int lastComponent)
{
  int Index;
  double Coefficient;
  int m1;
...
ostream& operator << (ostream& Str, ParticleOnSphereCoulombHamiltonian& H) 
{
...
MathematicaOutput& operator << (MathematicaOutput& Str, ParticleOnSphereCoulombHamiltonian& H) 
{
  RealVector TmpV2 (H.Particles->GetHilbertSpaceDimension(), true);
  RealVector* TmpV = new RealVector [H.Particles->GetHilbertSpaceDimension()];
  for (int i = 0; i < H.Particles->GetHilbertSpaceDimension(); i++)
