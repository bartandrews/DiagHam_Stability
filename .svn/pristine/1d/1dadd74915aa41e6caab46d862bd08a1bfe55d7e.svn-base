...
int main(int argc, char** argv)
{
  ...
  SingleIntegerOption NbrEigenvaluesOption ('n', "nbr-eigen", "number of eigenvalues", 10);
...
//  Max = 30;
  ...
  for (; L <= Max; L += 2)
	...
	Architecture = new SMPArchitecture(NbrProcessor);
      ...
      if (DeltaFlag == false)
	Hamiltonian = new ParticleOnSphereCoulombHamiltonian(&Space, NbrBosons, LzMax, Architecture, Memory);
      else
	Hamiltonian = new ParticleOnSphereCoulombDeltaHamiltonian(&Space, NbrBosons, LzMax, CoulombRatio, Architecture, Memory);
