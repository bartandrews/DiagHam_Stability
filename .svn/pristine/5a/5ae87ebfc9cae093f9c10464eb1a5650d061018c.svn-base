...
FermionOnSphereWithSpinAndPairing::FermionOnSphereWithSpinAndPairing()
{
}
...
FermionOnSphereWithSpinAndPairing::FermionOnSphereWithSpinAndPairing (int totalLz, int lzMax, int totalSpin, unsigned long memory)
{
  this->NbrFermions = 0;
  ...
  this->LargeHilbertSpaceDimension = this->EvaluateHilbertSpaceDimension(this->LzMax, 0, 0);
...
FermionOnSphereWithSpinAndPairing::FermionOnSphereWithSpinAndPairing(const FermionOnSphereWithSpinAndPairing& fermions)
{
...
FermionOnSphereWithSpinAndPairing& FermionOnSphereWithSpinAndPairing::operator = (const FermionOnSphereWithSpinAndPairing& fermions)
{
...
AbstractHilbertSpace* FermionOnSphereWithSpinAndPairing::Clone()
{
  return new FermionOnSphereWithSpinAndPairing(*this);
}
...
long FermionOnSphereWithSpinAndPairing::GenerateStates(int lzMax, int totalLz, int totalSpin, long pos)
{
  if (lzMax < 0)
    {
      if ((totalLz == this->TotalLz) && (totalSpin == this->TotalSpin))
	{
  ...
  for (; pos < TmpPos; ++pos)
    this->StateDescription[pos] |= Mask;
  return this->GenerateStates(lzMax - 1, totalLz, totalSpin,  pos);
}
...
long FermionOnSphereWithSpinAndPairing::EvaluateHilbertSpaceDimension(int lzMax, int totalLz,int totalSpin)
{
  if (lzMax < 0)
    {
      if ((totalLz == this->TotalLz) && (totalSpin == this->TotalSpin))
