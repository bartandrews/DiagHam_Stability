...
//      NbrStates + NbrBosons - 1 < 127 or 63 (64 bit or 32bit systems)        //
//                                                                            //
...
BosonOnLatticeLong::BosonOnLatticeLong ()
{
...
BosonOnLatticeLong::BosonOnLatticeLong (int nbrBosons, int lx, int ly, int nbrFluxQuanta, unsigned long memory, double solenoidX, double solenoidY, char landauGaugeAxis)
{
  ...
  this->HardCoreBasis = new HardCoreBosonLong(nbrBosons, NbrStates + nbrBosons - 1, memory);
...
BosonOnLatticeLong::BosonOnLatticeLong(const BosonOnLatticeLong& bosons)
{
  ...
  this->HardCoreBasis = (HardCoreBosonLong*) bosons.HardCoreBasis->Clone();
...
BosonOnLatticeLong& BosonOnLatticeLong::operator = (const BosonOnLatticeLong& bosons)
{
  if (this->HardCoreBasis != 0)
    {
      delete this->HardCoreBasis;
...
AbstractHilbertSpace* BosonOnLatticeLong::Clone()
{
  return new BosonOnLatticeLong(*this);
}
...
List<AbstractQuantumNumber*> BosonOnLatticeLong::GetQuantumNumbers ()
{
...
AbstractQuantumNumber* BosonOnLatticeLong::GetQuantumNumber (int index)
{
...
AbstractHilbertSpace* BosonOnLatticeLong::ExtractSubspace (AbstractQuantumNumber& q, 
...
int BosonOnLatticeLong::GetNbrSites()
{
...
void BosonOnLatticeLong::SetNbrFluxQuanta(int nbrFluxQuanta)
{
  ...
  switch (this->LandauGaugeAxis)
    ...
    case 'y':
      ...
      break;
    default:
      cout << "Unknown Quantization axis! Exiting BosonOnLatticeLong..."<<endl;
      exit(1);
      break;
    }
}
...
void BosonOnLatticeLong::SetNbrFluxQuanta(int nbrFluxQuanta, double solenoidX, double solenoidY)
{
...
// solenoidY = new solenoid flux through torus in y-direction
//
void BosonOnLatticeLong::GetSolenoidFluxes(double &solenoidX, double &solenoidY)
{
...
int BosonOnLatticeLong::GetNbrFluxQuanta()
{
...
// state = word to be acted upon
// q = quantum number of boson to be added
unsigned long BosonOnLatticeLong::Ad (unsigned long state, int q, double& coefficient)
{
  cout <<"using the wrong Ad function in BosonOnLatticeLong"<<endl;
  return 0x0ul;
}
...
int BosonOnLatticeLong::AdAdAA (int index, int q1, int q2, int r1, int r2, double& coefficient)
{
...
double BosonOnLatticeLong::AA (int index, int r1, int r2)
{
...
int BosonOnLatticeLong::AdAd (int q1, int q2, double& coefficient)
{
...
double BosonOnLatticeLong::AdA (int index, int q)
{
  this->FermionToBoson(this->HardCoreBasis->StateDescription[index], this->HardCoreBasis->StateHighestBit[index], this->TemporaryState, this->TemporaryStateHighestBit);
  if (q > this->NbrStates)  
    return 0.0;
...
// coefficient = reference on the double where the multiplicative factor has to be stored
// return value = index of the destination state 
int BosonOnLatticeLong::AdA (int index, int q, int r, double& coefficient)
{
...
//
double BosonOnLatticeLong::AdAdAADiagonal(int index, int nbrInteraction, double *interactionPerQ, int *qValues)
{
  this->FermionToBoson(this->HardCoreBasis->StateDescription[index], this->HardCoreBasis->StateHighestBit[index], this->TemporaryState, this->TemporaryStateHighestBit);
...
//
/*
double BosonOnLatticeLong::ProdAdProdADiagonal(int index, int nbrOperators, int nbrInteraction, double *interactionPerQ, int *qValues)
{
  this->FermionToBoson(this->HardCoreBasis->StateDescription[index], this->HardCoreBasis->StateHighestBit[index], this->TemporaryState, this->TemporaryStateHighestBit);
  int BosonsLeft=this->NbrBosons;
  double result=0.0;
...
// 
int BosonOnLatticeLong::EncodeQuantumNumber(int posx, int posy, int sublattice, Complex &translationPhase)
{
...
// posx = position along x-direction
// posy = position along y-direction
void BosonOnLatticeLong::DecodeQuantumNumber(int q, int &posx, int &posy, int &sublattice)
{
...
//
bool BosonOnLatticeLong::HaveOrder ()
{
  return true;
}
...
int BosonOnLatticeLong::CheckOrder (int* m, int* n, int nbrIndices)
{
...
void BosonOnLatticeLong::ListQuantumNumbers(int index, int *quantumNumbers, double &normalization)
{
  this->FermionToBoson(this->HardCoreBasis->StateDescription[index], this->HardCoreBasis->StateHighestBit[index], this->TemporaryState, this->TemporaryStateHighestBit);
  int NbrQ=0;
  normalization=1.0;
...
// obtain a list of quantum numbers in state
// quantumNumbers = integer array of length NbrParticles, to be written with quantum numbers of individual particles
void BosonOnLatticeLong::ListQuantumNumbers(int index, int *quantumNumbers)
{
  this->FermionToBoson(this->HardCoreBasis->StateDescription[index], this->HardCoreBasis->StateHighestBit[index], this->TemporaryState, this->TemporaryStateHighestBit);
  int NbrQ=0;
  for (int q=0; q<=TemporaryStateHighestBit; ++q)
...
int BosonOnLatticeLong::TranslateState(int index, int shiftX, int shiftY, Complex &translationPhase)
{
  this->FermionToBoson(this->HardCoreBasis->StateDescription[index], this->HardCoreBasis->StateHighestBit[index], this->TemporaryState, this->TemporaryStateHighestBit);
  int BosonsLeft=this->NbrBosons;
  int Q=TemporaryStateHighestBit;
...
bool BosonOnLatticeLong::IsTranslation(int i, int f, int &shiftX, int &shiftY)
{
...
ostream& BosonOnLatticeLong::PrintState (ostream& Str, int state)
{
...
int BosonOnLatticeLong::CarefulFindStateIndex(ULONGLONG stateDescription, int highestBit)
{
...
// highestBit = maximum nonzero bit reached by a particle in the state (can be given negative, if not known)
// return value = corresponding index, or dimension of space, if not found
int BosonOnLatticeLong::CarefulFindStateIndex(unsigned long stateDescription, int highestBit)
{
  cout <<"Using the wrong CarefulFindStateIndex function in BosonOnLatticeLong"<<endl;
  return this->HilbertSpaceDimension;
}
...
HermitianMatrix BosonOnLatticeLong::EvaluatePartialDensityMatrixParticlePartition (int nbrParticleSector, ComplexVector& groundState, AbstractArchitecture* architecture)
{
  ...
  BosonOnLatticeLong SubsytemSpace (nbrParticleSector, this->Lx, this->Ly, this->NbrFluxQuanta, 10000000, this->SolenoidX, this->SolenoidY);
  HermitianMatrix TmpDensityMatrix (SubsytemSpace.GetHilbertSpaceDimension(), true);
  BosonOnLatticeLong ComplementarySpace (ComplementaryNbrParticles, this->Lx, this->Ly, this->NbrFluxQuanta, 10000000, this->SolenoidX, this->SolenoidY);
...
long BosonOnLatticeLong::EvaluatePartialDensityMatrixParticlePartitionCore (int minIndex, int nbrIndex, ParticleOnLattice* complementaryHilbertSpace,  ParticleOnLattice* destinationHilbertSpace,
									  ComplexVector& groundState,  HermitianMatrix* densityMatrix)
{
   BosonOnLatticeLong* TmpHilbertSpace =  (BosonOnLatticeLong*) complementaryHilbertSpace;
   BosonOnLatticeLong* TmpDestinationHilbertSpace =  (BosonOnLatticeLong*) destinationHilbertSpace;
   ...
   for (; minIndex < MaxIndex; ++minIndex)    
      ...
      for (int j = 0; j < TmpDestinationHilbertSpace->HilbertSpaceDimension; ++j)
	{
	  ...
	  ULONGLONG TmpState = this->ConvertFromMonomial(TmpMonomial3);
...
void BosonOnLatticeLong::SymmetrizeU1U1State (ComplexVector& symmetrizedVector, ComplexVector& leftVector, ComplexVector& rightVector, BosonOnLatticeLong* leftSpace, BosonOnLatticeLong* rightSpace, bool unnormalizedBasisFlag, unsigned long firstComponent, unsigned long nbrComponents)
{
