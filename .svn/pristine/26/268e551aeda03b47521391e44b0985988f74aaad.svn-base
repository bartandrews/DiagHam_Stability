...
FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong (int nbrFermions, int nbrSiteX, int nbrSiteY, int kxMomentum, int kyMomentum, unsigned long memory)
{  
...
FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong (int nbrFermions, int nbrSiteX, int nbrSiteY, int kxMomentum, int kyMomentum,
...
FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong (int nbrFermions, int nbrSiteX, int nbrSiteY,
...
FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong(const FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong& fermions)
{
...
FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::~FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong ()
{
}
...
FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong& FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::operator = (const FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong& fermions)
{
...
AbstractHilbertSpace* FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::Clone()
{
  return new FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong(*this);
}
...
ostream& FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::PrintState (ostream& Str, int state)
{
  ...
  for (int i = 0; i < this->NbrLzValue; ++i)
    {
      ...
      for (int j = 0; j < 8; ++j)
	{
	  if ((Tmp & (((ULONGLONG) 0x1ul) << j)) != ((ULONGLONG) 0x0ul))
	    Str << "(" << TmpKx << "," << TmpKy << "," << (j+1) << ")";
	}
    }
  Str << "]";
...
long FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::GenerateStates(int nbrFermions, int currentKx, int currentKy, int currentTotalKx, int currentTotalKy, long pos)
{
  if (currentKy < 0)
    {
  ...
  for (int i = 255; i >= 0; --i)
    {
      ...
      Mask = ((ULONGLONG) i) << (((currentKx * this->NbrSiteY) + currentKy) << 3);
      for (; pos < TmpPos; ++pos)
...
long FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::GenerateStates(int nbrFermions, int currentKx, int currentKy, int currentTotalKx, int currentTotalKy,
								    ...
								    int nbrParticles5, int nbrParticles6, int nbrParticles7, int nbrParticles8, long pos)
{
  ...
  if (nbrFermions == 1)
    {
      ULONGLONG TmpMask = ((ULONGLONG) (nbrParticles1 | (nbrParticles2 << 1) | (nbrParticles3 << 2) | (nbrParticles4 << 3)
								    ...
								    int nbrParticles12, int nbrParticles34, int nbrParticles56, int nbrParticles78, long pos)
{
  ...
  if (nbrFermions == 1)
    {
      ULONGLONG TmpMask = ((ULONGLONG) (nbrParticles12 | (nbrParticles34 << 2) | (nbrParticles56 << 4) | (nbrParticles78 << 6)));
      for (int j = currentKy; j >= 0; --j)
	{
	  if ((((currentKx + currentTotalKx) % this->NbrSiteX) == this->KxMomentum) &&
	      (((j + currentTotalKy) % this->NbrSiteY) == this->KyMomentum))
...
long FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::GenerateStates(int nbrParticles1, int nbrParticles2, int nbrParticles3, int nbrParticles4,
								    int nbrParticles5, int nbrParticles6, int nbrParticles7, int nbrParticles8)
{
  ...
  ULONGLONG*** HilbertSpacePerKSector = new  ULONGLONG** [MaxNbrParticlesPerSector+ 1];
  ...
  for (int TmpN = MinNbrParticlesPerSector; TmpN <= MaxNbrParticlesPerSector; ++TmpN)
	  ...
	  || (TmpN == nbrParticles5) || (TmpN == nbrParticles6) || (TmpN == nbrParticles7) || (TmpN == nbrParticles8))
	{
	  ...
	  HilbertSpacePerKSector[TmpN] = new ULONGLONG* [(MaxTotalKx + 1) * (MaxTotalKy + 1)];
		  ...
		  if (TmpDimension > 0l)
		    {
		      ...
		      HilbertSpacePerKSector[TmpN][NbrDimensionPerKSector[TmpN]] = new ULONGLONG[TmpDimension];
			  ...
			  for (int i2 = 0; i2 < NbrDimensionPerKSector[nbrParticles2]; ++i2)
			      ...
			      for (int i1 = 0; i1 < NbrDimensionPerKSector[nbrParticles1]; ++i1)
				{
				  ...
				  if ((TmpKx == this->KxMomentum) && (TmpKy == this->KyMomentum))
					...
					{
					  ULONGLONG Mask8 = HilbertSpacePerKSector[nbrParticles8][i8][j8] << 7;
					  for (int j7 = 0; j7 < Dim7; ++j7)
					    {
					      ULONGLONG Mask7 = Mask8 | (HilbertSpacePerKSector[nbrParticles7][i7][j7] << 6);
					      for (int j6 = 0; j6 < Dim6; ++j6)
						{
						  ULONGLONG Mask6 = Mask7 | (HilbertSpacePerKSector[nbrParticles6][i6][j6] << 5);
						  for (int j5 = 0; j5 < Dim5; ++j5)
						    {
						      ULONGLONG Mask5 = Mask6 | (HilbertSpacePerKSector[nbrParticles5][i5][j5] << 4);
						      for (int j4 = 0; j4 < Dim4; ++j4)
							{
							  ULONGLONG Mask4 = Mask5 | (HilbertSpacePerKSector[nbrParticles4][i4][j4] << 3);
							  for (int j3 = 0; j3 < Dim3; ++j3)
							    {
							      ULONGLONG Mask3 = Mask4 | (HilbertSpacePerKSector[nbrParticles3][i3][j3] << 2);
							      for (int j2 = 0; j2 < Dim2; ++j2)
								{
								  ULONGLONG Mask2 = Mask3 | (HilbertSpacePerKSector[nbrParticles2][i2][j2] << 1);
  ...
  SortArrayDownOrdering<ULONGLONG>(this->StateDescription, TmpDimension);
  for (int TmpN = MinNbrParticlesPerSector; TmpN <= MaxNbrParticlesPerSector; ++TmpN)
    {
...
long FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::EvaluateHilbertSpaceDimension(int nbrFermions, int currentKx, int currentKy, int currentTotalKx, int currentTotalKy)
{
  if (currentKy < 0)
    {
      currentKy = this->NbrSiteY - 1;
...
long FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::EvaluateHilbertSpaceDimension(int nbrParticles1, int nbrParticles2, int nbrParticles3, int nbrParticles4,
										   int nbrParticles5, int nbrParticles6, int nbrParticles7, int nbrParticles8)
{
  int TotalNbrSites = this->NbrSiteX * this->NbrSiteY;
  int MinNbrParticlesPerSector = 0;
...
long FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::GenerateStatesOneBand(ULONGLONG* stateDescriptions, int nbrFermions,
									   int kxMomentum, int kyMomentum,
  ...
  if (nbrFermions == 0)
    {
      if ((currentTotalKx == kxMomentum) && (currentTotalKy == kyMomentum))
        {
	  stateDescriptions[pos] = ((ULONGLONG) 0x0ul);	  
	  return (pos + 1l);
        }
      else
	{
...
long FermionOnSquareLatticeWithSU8SpinMomentumSpaceLong::EvaluateHilbertSpaceDimensionOneBand(int nbrParticles, int kxMomentum, int kyMomentum,
