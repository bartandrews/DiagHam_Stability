...
{
  OptionManager Manager ("FQHESphereTwoLandauLevelsEntanglementEntropyParticlePartition" , "0.01");
  ...
  (*SystemGroup) += new BooleanOption  ('\n', "use-svd", "use singular value decomposition instead of diagonalization to compute the entropy",true);
  ...
  if (Manager.ProceedOptions(argv, argc, cout) == false)
    {
      cout << "see man page for option syntax or type FQHESphereBosonTwoLandauLevelsEntanglementEntropyParticlePartition -h" << endl;
      return -1;
    }
	...
	if ((LzMax + NbrParticles - 1) < 31)	
#endif
	  {
	    if (Manager.GetBoolean("haldane") == false)
	      {
		Spaces[i] = new BosonOnSphereTwoLandauLevels (NbrParticles, TotalLz[i], LzMax + 2 , LzMax);
	      }
	    /*else
	      {
  ...
  for (; SubsystemNbrParticles <= MaxSubsystemNbrParticles; ++SubsystemNbrParticles)
    {
      ...
      int SubsystemMaxTotalLz = SubsystemNbrParticles * (LzMax+2);
      int SubsystemTotalLz = -SubsystemMaxTotalLz; 
      if (LargestLSector == true)
	{
	  if ((((LzMax + 2) * NbrParticles) & 1) == 0)
	    {
	      SubsystemTotalLz = 0;
