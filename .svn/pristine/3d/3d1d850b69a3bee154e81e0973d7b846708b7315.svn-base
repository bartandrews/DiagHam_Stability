...
int main(int argc, char** argv)
{
  ...
  OptionManager Manager ("FQHETorusFermionsFluctuatingMetric" , "0.01");
  ...
  if (Manager.GetString("interaction-file") == 0)
    ...
    {
      if (FQHETorusGetPseudopotentials(Manager.GetString("interaction-file"), NbrPoints, MetricField) == false)
	return -1;
    }
  for (int i = 0; i < NbrPoints; ++i)
    cout<<"V["<<i<<"]= "<<MetricField[i]<<" ";
  cout<<endl;
  ...
  for (; Ky <= Max; Ky+=2 )
	...
	Memory = Architecture.GetArchitecture()->GetLocalMemory();
      ...
      AbstractQHEHamiltonian* Hamiltonian = new ParticleOnCylinderFluctuatingMetricHamiltonian (Space, NbrParticles, MaxMomentum, XRatio, NbrPoints, MetricField, Architecture.GetArchitecture(), Memory);
