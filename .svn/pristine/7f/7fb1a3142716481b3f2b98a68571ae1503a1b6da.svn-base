...
SpinChainRealFullHamiltonian::SpinChainRealFullHamiltonian(AbstractSpinChain* chain, int nbrSpin, double* jx, double* jy, double* jz, 
							   double* hx, double* hz, bool periodicBoundaryConditions)
{
  this->Chain = chain;
  ...
  this->HxFactor = new double [this->NbrSpin];
  this->HzFactor = new double [this->NbrSpin];
  for (int i = 0; i < this->NbrSpin; i++)
    {
      this->HxFactor[i] = 0.5 * hx[i];
...
SpinChainRealFullHamiltonian::~SpinChainRealFullHamiltonian() 
{
...
void SpinChainRealFullHamiltonian::SetHilbertSpace (AbstractHilbertSpace* hilbertSpace)
{
  delete[] this->SzSzContributions;
...
AbstractHilbertSpace* SpinChainRealFullHamiltonian::GetHilbertSpace ()
{
  return this->Chain;
}
...
int SpinChainRealFullHamiltonian::GetHilbertSpaceDimension ()
{
...
void SpinChainRealFullHamiltonian::ShiftHamiltonian (double shift)
{
...
RealVector& SpinChainRealFullHamiltonian::SpinChainRealFullHamiltonian::LowLevelAddMultiply(RealVector& vSource, RealVector& vDestination, 
											       int firstComponent, int nbrComponent)
{
  ...
  for (int i = firstComponent; i < LastComponent; ++i)
      ...
      for (int j = 0; j < MaxPos; ++j)
	{
	  ...
	  if (pos != dim)
	    {
	      vDestination[pos] += coef * this->HxFactor[j] * TmpValue;
	    }
      ...
      if (this->PeriodicBoundaryConditions == true)
	{
      ...
      if (pos != dim)
	{
	  vDestination[pos] += coef * this->HxFactor[MaxPos] * TmpValue;
	}
...
RealVector* SpinChainRealFullHamiltonian::LowLevelMultipleAddMultiply(RealVector* vSources, RealVector* vDestinations, int nbrVectors, 
									 int firstComponent, int nbrComponent)
{
  int LastComponent = firstComponent + nbrComponent;
  int dim = this->Chain->GetHilbertSpaceDimension();
  ...
  for (int i = firstComponent; i < LastComponent; ++i)
      ...
      for (int j = 0; j < MaxPos; ++j)
	{
	  ...
	  if (pos != dim)
	    {
	      for (int k = 0; k < nbrVectors; ++k)
		vDestinations[k][pos] += coef * this->HxFactor[j] * TmpValues[k];
	    }
	  pos = this->Chain->Smi(j, i, coef);
	  if (pos != dim)
	    {
	...
	{
	  for (int k = 0; k < nbrVectors; ++k)
	    vDestinations[k][pos] += coef * this->HxFactor[MaxPos] * TmpValues[k];
	}
      pos = this->Chain->Smi(MaxPos, i, coef);
      if (pos != dim)
	{
...
void SpinChainRealFullHamiltonian::EvaluateDiagonalMatrixElements()
{
  int dim = this->Chain->GetHilbertSpaceDimension();
