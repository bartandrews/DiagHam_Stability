...
#include "Hamiltonian/ParticleOnThinAnnulusGenericHamiltonian.h"
...
int main(int argc, char** argv)
{
  ...
  OptionManager Manager ("FQHEThinAnnulusFermionsTwoBodyGeneric" , "0.01");
 ...
 (*MiscGroup) += new BooleanOption  ('h', "help", "display this help");
  ...
  if (Manager.ProceedOptions(argv, argc, cout) == false)
    {
      cout << "see man page for option syntax or type FQHEThinAnnulusFermionsTwoBodyGeneric -h" << endl;
      return -1;
    }
  ...
  if (ForceMaxMomentum >= 0)
    sprintf (OutputNameLz, "fermions_thinannulus_%s_n_%d_lzmax_%d_lz_%d.dat", Manager.GetString("interaction-name"), NbrParticles, ForceMaxMomentum, MMax);
  else
    sprintf (OutputNameLz, "fermions_thinannulus_%s_n_%d_lz_%d.dat", Manager.GetString("interaction-name"), NbrParticles, MMax);
  for (int  L = MMin; L <= MMax; ++L)
	...
	Hamiltonian = new ParticleOnThinAnnulusGenericHamiltonian(Space, NbrParticles, TmpMaxMomentum, PseudoPotentials,
							   Architecture.GetArchitecture(), 
							   Memory, DiskCacheFlag,
							   LoadPrecalculationFileName);
      else
	Hamiltonian = new ParticleOnThinAnnulusGenericHamiltonian(Space, NbrParticles, TmpMaxMomentum, PseudoPotentials, OneBodyPotentials, MMax,
							   Architecture.GetArchitecture(), 
							   Memory, DiskCacheFlag,
							   LoadPrecalculationFileName);
      double Shift = 0.0;//- 0.5 * ((double) (NbrParticles * NbrParticles)) / (0.5 * ((double) MMax));
      ...
      if (Manager.GetBoolean("eigenstate") == true)	
	{
	  ...
	  if (ForceMaxMomentum >= 0)
	    sprintf (EigenvectorName, "fermions_thinannulus_%s_n_%d_lzmax_%d_lz_%d", Manager.GetString("interaction-name"), NbrParticles, ForceMaxMomentum, L);
	  else
	    sprintf (EigenvectorName, "fermions_thinannulus_%s_n_%d_lz_%d", Manager.GetString("interaction-name"), NbrParticles, L);
	}
