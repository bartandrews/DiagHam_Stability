...
void FCIPPlusIPSquareLatticeModelComputeInteractingOrbitals(int*& nbrInteractingOrbitals, int**& interactingOrbitalsOrbitalIndices,
...
{
  OptionManager Manager ("FCIPPlusIPSquareLatticeModel" , "0.01");
  ...
  (*SystemGroup) += new SingleDoubleOption  ('\n', "t2", "next nearest neighbor hoping amplitude", 0.5);
  ...
  if (Manager.ProceedOptions(argv, argc, cout) == false)
    {
      cout << "see man page for option syntax or type FCIPPlusIPSquareLatticeModel -h" << endl;
      return -1;
    }
  ...
  if (Manager.GetBoolean("singleparticle-spectrum") == true)
	...
	ExportOneBody = true;
      if (TiltedFlag == false)
	TightBindingModel = new TightBindingModelPPlusIPSquareLattice (NbrSitesX, NbrSitesY, Manager.GetDouble("t1"), Manager.GetDouble("t2"), 
								       Manager.GetDouble("mu-s"), Manager.GetDouble("gamma-x"), Manager.GetDouble("gamma-y"),
								       Architecture.GetArchitecture(), ExportOneBody);
      else
	TightBindingModel = new TightBindingModelPPlusIPSquareLattice (NbrSitesX, NbrSitesY, nx1, ny1, nx2, ny2, Offset, Manager.GetDouble("t1"), Manager.GetDouble("t2"), 
								       Manager.GetDouble("mu-s"), Manager.GetDouble("gamma-x"), Manager.GetDouble("gamma-y"),
								       Architecture.GetArchitecture(), OffsetReal, ExportOneBody);
...
// 	}
  ...
  if (Manager.GetString("import-onebody") == 0)
    {
      if (TiltedFlag == false)
	TightBindingModel = new TightBindingModelPPlusIPSquareLattice (NbrSitesX, NbrSitesY, Manager.GetDouble("t1"), Manager.GetDouble("t2"), Manager.GetDouble("mu-s"), Manager.GetDouble("gamma-x"), Manager.GetDouble("gamma-y"), Architecture.GetArchitecture(), true);
      else
      {
	TightBindingModel = new TightBindingModelPPlusIPSquareLattice (NbrSitesX, NbrSitesY, nx1, ny1, nx2, ny2, Offset,  Manager.GetDouble("t1"), Manager.GetDouble("t2"), Manager.GetDouble("mu-s"), Manager.GetDouble("gamma-x"), Manager.GetDouble("gamma-y"), Architecture.GetArchitecture(), OffsetReal, true);
      }
	  ...
	  cout << "(kx=" << i << ",ky=" << j << ") : " << endl;
 	  if (Manager.GetBoolean("single-band") == false)
		...
		Memory = Architecture.GetArchitecture()->GetLocalMemory();
	      ...
	      FCIPPlusIPSquareLatticeModelComputeInteractingOrbitals(NbrInteractingOrbitals, InteractingOrbitalsOrbitalIndices, 
