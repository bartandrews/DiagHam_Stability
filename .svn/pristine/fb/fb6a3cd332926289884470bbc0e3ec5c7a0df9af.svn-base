...
#include "Hamiltonian/ParticleOnCylinderFluctuatingMetricHamiltonian.h"
...
ParticleOnCylinderFluctuatingMetricHamiltonian::ParticleOnCylinderFluctuatingMetricHamiltonian(ParticleOnSphere* particles, int nbrParticles, int maxMomentum,
										   double ratio, int nbrPoints, double* metricField, AbstractArchitecture* architecture, long memory, char* precalculationFileName)
{
  ...
  this->NbrPointsMetricField = nbrPoints;
  this->MetricField = new double[this->NbrPointsMetricField];
  cout<<"Metric field: ";
  for (int i = 0; i < this->NbrPointsMetricField; ++i)
   {
    this->MetricField[i] = metricField[i];
    cout<<i<<" "<<this->MetricField[i]<<"; "; 
   }
  cout<<endl;
  if (this->NbrPointsMetricField != (2 * this->MaxMomentum + 1))
   {
     exit(1);
   }
...
ParticleOnCylinderFluctuatingMetricHamiltonian::~ParticleOnCylinderFluctuatingMetricHamiltonian() 
{
...
void ParticleOnCylinderFluctuatingMetricHamiltonian::SetHilbertSpace (AbstractHilbertSpace* hilbertSpace)
{
  delete[] this->InteractionFactors;
  if (this->FastMultiplicationFlag == true)
    {
...
void ParticleOnCylinderFluctuatingMetricHamiltonian::ShiftHamiltonian (double shift)
{
...
void ParticleOnCylinderFluctuatingMetricHamiltonian::EvaluateInteractionFactors()
{
  int Pos = 0;
  int m4;
...
Complex ParticleOnCylinderFluctuatingMetricHamiltonian::EvaluateInteractionCoefficient(int m1, int m2, int m3, int m4)
{
  ...
  double alpha = this->MetricField[m1 + m2];

  Coefficient.Re = exp(-pow(Xm1-Xm3,2.0)/(2.0*alpha) - pow(Xm1-Xm4,2.0)/(2.0*alpha)) * (pow(Xm1-Xm3,2.0)- pow(Xm1-Xm4,2.0));
  Coefficient.Im = 0.0;
