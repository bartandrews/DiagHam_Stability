...
#ifndef POTTS4CHAIN_H
#define POTTS4CHAIN_H
...
class Potts4Chain : public Potts3Chain
{
 ...
 public:
  ...
  //
  // chainLength = number of sites
  // memorySize = memory size in bytes allowed for look-up table
  Potts4Chain (int chainLength, int memorySize);

  // constructor for complete Hilbert space corresponding to a given total charge sector
  //
  // chainLength = number of spin 1
  // q = charge sector
  // memorySize = memory size in bytes allowed for look-up table
  Potts4Chain (int chainLength, int q, int memorySize) ;

  // copy constructor (without duplicating datas)
  //
  // chain = reference on chain to copy
  Potts4Chain (const Potts4Chain& chain);
  ...
  Potts4Chain& operator = (const Potts4Chain& chain);
  ...
  //
  // index = index of the state 
  // return value = Q value (0 for 1, 1 for exp(i 2 \pi / 4), -1 1 for exp(i 2 \pi / 4)) 
 ...
 protected:
  ...
  Potts4Chain (long largeHilbertSpaceDimension, unsigned long* chainDescription, int chainLength, 
...
// i = position of Sz operator
// state = index of the state to be applied on Sz_i operator
// coefficient = reference on double where numerical coefficient has to be stored (0 for 1.0, 1.0 for exp(i\pi / 2), 2.0 for exp(i \pi) and 3.0 for   exp(-i\pi / 2)
// return value = index of resulting state
...
//
// index = index of the state 
// return value = Q value (0 for 1, 1 for exp(i \pi / 2),  2 for exp(i \pi) and 3 for exp(-i \pi / 2)
