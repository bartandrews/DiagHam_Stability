	  ...
	  for (int j = 0; j <= this->LzMax; ++j)
	    {
	      ...
	      if (Index < Dim)
		{
		  vDestination[Index] += Coefficient * OneBodyInteractionFactorsdownup[j] * Source;
		}
  ...
  if (this->OneBodyInteractionFactorsupdown != 0)
      ...
      for (int i = firstComponent; i < lastComponent; i += step)
	{
	  for (int j = 0; j <= this->LzMax; ++j)
		...
		{
		  for (int p = 0; p < nbrVectors; ++p)
		    vDestinations[p][Index] += Coefficient * OneBodyInteractionFactorsdownup[j] * vSources[p][i];
		}
	      Index = particles->AduAd(i + this->PrecalculationShift, j, j, Coefficient);
	      if (Index < Dim)
		{
