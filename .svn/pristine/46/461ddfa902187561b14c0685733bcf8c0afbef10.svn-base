...
Spin1ChainWithTranslations::Spin1ChainWithTranslations (int chainLength, int momentum, unsigned long memory) 
{
  this->Flag.Initialize();
...
Spin1ChainWithTranslations::Spin1ChainWithTranslations (int chainLength, int momentum, int sz, unsigned long memory) 
{
  this->Flag.Initialize();
  this->ChainLength = chainLength;
  this->Sz = sz;
...
int Spin1ChainWithTranslations::SpiSmi (int i, int state, double& coefficient, int& nbrTranslation)
{
  unsigned long TmpState = (this->StateDescription[state] >> (i << 1)) & 0x3ul;
  if (TmpState == 0x0ul)
    {
      return this->HilbertSpaceDimension;
    }
...
int Spin1ChainWithTranslations::SmiSpi (int i, int state, double& coefficient, int& nbrTranslation)
{
  unsigned long TmpState = (this->StateDescription[state] >> (i << 1)) & 0x3ul;
  if (TmpState == 0x3ul)
    {
...
int Spin1ChainWithTranslations::SmiSpjSmiSpj (int i1, int j1, int i2, int j2, int state, double& coefficient, int& nbrTranslation)
{
  unsigned long TmpState = this->StateDescription[state];
  j2 <<= 1;
...
int Spin1ChainWithTranslations::SmiSmjSpiSpj (int i1, int j1, int i2, int j2, int state, double& coefficient, int& nbrTranslation)
{
  unsigned long TmpState = this->StateDescription[state];
  j2 <<= 1;
  switch ((TmpState >> j2) & 0x3ul)
...
int Spin1ChainWithTranslations::SziSzjSmiSpj (int i1, int j1, int i2, int j2, int state, double& coefficient, int& nbrTranslation)
{
  unsigned long TmpState = this->StateDescription[state];
  j2 <<= 1;
  switch ((TmpState >> j2) & 0x3ul)
...
int Spin1ChainWithTranslations::FindStateIndex(unsigned long stateDescription, int maxBitPosition)
{
...
long Spin1ChainWithTranslations::EvaluateHilbertSpaceDimension(int nbrSites)
{
  long Tmp = 3l;
  for (int i = 1; i < this->ChainLength; ++i)
    {
...
long Spin1ChainWithTranslations::EvaluateHilbertSpaceDimension(int sz, int nbrSites)
{
  if (nbrSites == 0)
    {
      if (sz == this->Sz)
...
long Spin1ChainWithTranslations::RawGenerateStates(long statePosition, int sitePosition) 
{
  if (sitePosition < 0)
    {
...
long Spin1ChainWithTranslations::RawGenerateStates(long statePosition, int sitePosition, int currentSz) 
{
  if (sitePosition < 0)
    {
...
long Spin1ChainWithTranslations::GenerateStates()
{
...
void Spin1ChainWithTranslations::GenerateLookUpTable(unsigned long memory)
{
...
void Spin1ChainWithTranslations::ComputeRescalingFactors()
{
...
ComplexMatrix Spin1ChainWithTranslations::EvaluatePartialEntanglementMatrix (int nbrSites, int szSector, ComplexVector& groundState, AbstractArchitecture* architecture)
{
  if (nbrSites == 0)
    {
      if (szSector == 0)
