...
#ifndef PARTICLEONLATTICEWITHSU3SPINCHERNINSULATORHAMILTONIAN_H
#define PARTICLEONLATTICEWITHSU3SPINCHERNINSULATORHAMILTONIAN_H
...
class ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian : public AbstractQHEHamiltonian
{
 ...
 protected:
  ...
  // array that contains all one-body interaction factors for tunnelling terms for particles with spin 1 and spin 2 
  Complex* OneBodyInteractionFactors12;
  // array that contains all one-body interaction factors for tunnelling terms for particles with spin 1 and spin 3
  Complex* OneBodyInteractionFactors13;
  // array that contains all one-body interaction factors for tunnelling terms for particles with spin 2 and spin 3
 ...
 public:
  ...
  // default constructor
  //
  ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian();
  ...
  ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian(ParticleOnSphereWithSU3Spin* particles, int nbrParticles, int nbrSiteX, int nbrSiteY, double kineticFactor, 
							double uPotential, double bandParameter, bool flatBandFlag, AbstractArchitecture* architecture, long memory = -1);

  // destructor
  //
  virtual ~ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian();
...
inline void ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian::EvaluateMNTwoBodyAddMultiplyComponent(ParticleOnSphereWithSU3Spin* particles, int index, ComplexVector& vSource, ComplexVector& vDestination)
{
...
inline void ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian::EvaluateMNTwoBodyAddMultiplyComponent(ParticleOnSphereWithSU3Spin* particles, int index, ComplexVector* vSources, 
													 ComplexVector* vDestinations, int nbrVectors, Complex* tmpCoefficients)
{
  int Dim = particles->GetHilbertSpaceDimension();
  double Coefficient;
...
inline void ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian::HermitianEvaluateMNTwoBodyAddMultiplyComponent(ParticleOnSphereWithSU3Spin* particles, int index, ComplexVector& vSource, ComplexVector& vDestination)
{
  //int Dim = particles->GetHilbertSpaceDimension();
  double Coefficient;
  double Coefficient3;
...
inline void ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian::HermitianEvaluateMNTwoBodyAddMultiplyComponent(ParticleOnSphereWithSU3Spin* particles, int index, ComplexVector* vSources, 
														  ComplexVector* vDestinations, int nbrVectors, Complex* tmpCoefficients)
{
  //int Dim = particles->GetHilbertSpaceDimension();
  double Coefficient;
...
inline void ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian::EvaluateMNTwoBodyFastMultiplicationComponent(ParticleOnSphereWithSU3Spin* particles, int index, 
														int* indexArray, Complex* coefficientArray, long& position)
{
  int Index;
...
inline void ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian::EvaluateMNOneBodyAddMultiplyComponent(ParticleOnSphereWithSU3Spin* particles, int firstComponent, int lastComponent,
												      int step, ComplexVector& vSource, ComplexVector& vDestination)
{
	  ...
	  if (this->OneBodyInteractionFactors33 != 0)
	      ...
	      for (int p = 0; p < nbrVectors; ++p)
		{
		  ...
		  for (int i = firstComponent; i < lastComponent; i += step)
			...
			{
			  TmpDiagonal += this->OneBodyInteractionFactors22[j] * particles->Ad2A2(i, j);
			  TmpDiagonal += this->OneBodyInteractionFactors33[j] * particles->Ad2A2(i, j);
			}
		      TmpDestinationVector[i] += (this->HamiltonianShift + TmpDiagonal)* TmpSourceVector[i];
		    }
		}
...
inline void ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian::EvaluateMNOneBodyFastMultiplicationComponent(ParticleOnSphereWithSU3Spin* particles, int index, 
													     int* indexArray, Complex* coefficientArray, long& position)
{
  if ((this->OneBodyInteractionFactors22 != 0) || (this->OneBodyInteractionFactors11 != 0))
    {
      double TmpDiagonal = 0.0;
      if (this->OneBodyInteractionFactors11 != 0)
	for (int j = 0; j <= this->LzMax; ++j)
	...
	for (int j = 0; j <= this->LzMax; ++j)
      ...
      if (this->OneBodyInteractionFactors33 != 0)
	for (int j = 0; j <= this->LzMax; ++j)
	  TmpDiagonal += this->OneBodyInteractionFactors22[j] * particles->Ad3A3(index + this->PrecalculationShift, j);	  
...
inline void ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian::EvaluateMNTwoBodyFastMultiplicationMemoryComponent(ParticleOnSphereWithSU3Spin* particles, int firstComponent, int lastComponent, long& memory)
{
  int Index;
  double Coefficient = 0.0;
  double Coefficient2 = 0.0;
...
inline void ParticleOnLatticeWithSU3SpinChernInsulatorHamiltonian::EvaluateMNOneBodyFastMultiplicationMemoryComponent(ParticleOnSphereWithSU3Spin* particles, int firstComponent, int lastComponent, long& memory)
{
  int Index;
  double Coefficient = 0.0;
	  ...
	  for (int i = firstComponent; i < lastComponent; ++i)
	    {
	      for (int j=0; j<= this->LzMax; ++j)
		{
		  Index = particles->Ad2A1(i, j, j, Coefficient);
		  if (Index < Dim)
		    {
		      ++memory;
		      ++this->NbrInteractionPerComponent[i - this->PrecalculationShift];	  
		    }
		  Index = particles->Ad1A2(i, j, j, Coefficient);
		  if (Index < Dim)
		    {
		      ++memory;
		      ++this->NbrInteractionPerComponent[i - this->PrecalculationShift];	  
	  ...
	  for (int i = firstComponent; i < lastComponent; ++i)
	    {
	      for (int j=0; j<= this->LzMax; ++j)
		{
		  Index = particles->Ad3A1(i, j, j, Coefficient);
		  if (Index < Dim)
		    {
		      ++memory;
		      ++this->NbrInteractionPerComponent[i - this->PrecalculationShift];	  
		    }
		  Index = particles->Ad1A3(i, j, j, Coefficient);
		  if (Index < Dim)
		    {
		      ++memory;
		      ++this->NbrInteractionPerComponent[i - this->PrecalculationShift];	  
	  ...
	  for (int i = firstComponent; i < lastComponent; ++i)
	    {
	      for (int j=0; j<= this->LzMax; ++j)
		{
		  Index = particles->Ad3A2(i, j, j, Coefficient);
		  if (Index < Dim)
		    {
		      ++memory;
		      ++this->NbrInteractionPerComponent[i - this->PrecalculationShift];	  
		    }
		  Index = particles->Ad2A3(i, j, j, Coefficient);
		  if (Index < Dim)
		    {
		      ++memory;
		      ++this->NbrInteractionPerComponent[i - this->PrecalculationShift];	  
