...
#ifndef ABSTRACTQHEONTORUSWITHMAGNETICTRANSLATIONSREALHAMILTONIAN_H
#define ABSTRACTQHEONTORUSWITHMAGNETICTRANSLATIONSREALHAMILTONIAN_H
...
class AbstractQHEOnTorusWithMagneticTranslationsRealHamiltonian : public AbstractQHEOnSphereFullHamiltonian
{
 ...
 protected:
  ...
  double* ExponentialFactors;

 public:

  // destructor
  //
  virtual ~AbstractQHEOnTorusWithMagneticTranslationsRealHamiltonian();
 ...
 protected:
  ...
  virtual void EvaluateMNTwoBodyAddMultiplyComponent(ParticleOnSphere* particles, int index, RealVector& vSource, RealVector& vDestination);
  ...
  virtual void EvaluateMNTwoBodyAddMultiplyComponent(ParticleOnSphere* particles, int index, RealVector* vSources, 
  ...
  // vSource = vector to be multiplied
  // vDestination = vector at which result has to be added  
  virtual void HermitianEvaluateMNTwoBodyAddMultiplyComponent(ParticleOnSphere* particles, int index, RealVector& vSource, RealVector& vDestination);
  ...
  // nbrVectors = number of vectors that have to be evaluated together
  // tmpCoefficients = a temporary array whose size is nbrVectors
  inline void HermitianEvaluateMNTwoBodyAddMultiplyComponent(ParticleOnSphere* particles, int index, RealVector* vSources, 
							     RealVector* vDestinations, int nbrVectors, double* tmpCoefficients);
...
inline void AbstractQHEOnTorusWithMagneticTranslationsRealHamiltonian::EvaluateMNTwoBodyAddMultiplyComponent(ParticleOnSphere* particles, int index, RealVector& vSource, RealVector& vDestination)
{
...
inline void AbstractQHEOnTorusWithMagneticTranslationsRealHamiltonian::EvaluateMNTwoBodyAddMultiplyComponent(ParticleOnSphere* particles, int index, RealVector* vSources, 
													      RealVector* vDestinations, int nbrVectors, double* tmpCoefficients)
{
  int Dim = particles->GetHilbertSpaceDimension();
  double Coefficient;
...
inline void AbstractQHEOnTorusWithMagneticTranslationsRealHamiltonian::HermitianEvaluateMNTwoBodyAddMultiplyComponent(ParticleOnSphere* particles, int index, RealVector& vSource, RealVector& vDestination)
{
  double Coefficient;
  double Coefficient3;
  double Coefficient4;
	  ...
	  if (Coefficient3 != 0.0)
	      ...
	      for (int i2 = 0; i2 < Lim; i2 += 2)
		{
		  if ((*TmpInteractionFactor) != 0.0)
			...
			{
			  if (Index < index)
			    TmpSum += vSource[Index] * (Coefficient * Coefficient3) * (this->ExponentialFactors[NbrTranslations] *  (*TmpInteractionFactor));
			  vDestination[Index] += (Coefficient * (*TmpInteractionFactor)) * (this->ExponentialFactors[NbrTranslations] * Coefficient4);
			}
		    }
		  ++TmpInteractionFactor;
...
inline void AbstractQHEOnTorusWithMagneticTranslationsRealHamiltonian::HermitianEvaluateMNTwoBodyAddMultiplyComponent(ParticleOnSphere* particles, int index, RealVector* vSources, 
														       RealVector* vDestinations, int nbrVectors, double* tmpCoefficients)
{
  double Coefficient;
  double Coefficient3;
		  ...
		  if ((*TmpInteractionFactor) != 0.0)
		      ...
		      if (Index <= index)
			{
			  if (Index < index)
			    {
			      Tmp = Coefficient * (*TmpInteractionFactor) * this->ExponentialFactors[NbrTranslations];
			      Tmp2 = (Coefficient * Coefficient3) * ((*TmpInteractionFactor) * this->ExponentialFactors[NbrTranslations]);
			      for (int p = 0; p < nbrVectors; ++p)
				{
				  vDestinations[p][Index] += Tmp * tmpCoefficients[p];
...
inline void AbstractQHEOnTorusWithMagneticTranslationsRealHamiltonian::EvaluateMNTwoBodyFastMultiplicationComponent(ParticleOnSphere* particles, int index, 
														  int* indexArray, double* coefficientArray, long& position)
{
  int Index;
...
inline void AbstractQHEOnTorusWithMagneticTranslationsRealHamiltonian::EvaluateMNTwoBodyFastMultiplicationMemoryComponent(ParticleOnSphere* particles, int firstComponent, int lastComponent, long& memory)
{
  int Index;
  double Coefficient = 0.0;
  double Coefficient2 = 0.0;
      ...
      for (int i = firstComponent; i < lastComponent; ++i)
	{
	  for (int j = 0; j < this->NbrSectorSums; ++j)
	      ...
	      for (int i1 = 0; i1 < Lim; i1 += 2)
		{
		  ...
		  if (Coefficient2 != 0.0)
		    {
		      double* TmpInteractionFactor = &(this->InteractionFactors[j][(i1 * Lim) >> 2]);
		      for (int i2 = 0; i2 < Lim; i2 += 2)
			{
			  if ((*TmpInteractionFactor) != 0.0)
			    {
