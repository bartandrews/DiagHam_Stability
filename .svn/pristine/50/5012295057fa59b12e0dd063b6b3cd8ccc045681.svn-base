...
#include "config.h"
#include "PairedCFOnSpherePermanentWaveFunction.h"
...
PairedCFOnSpherePermanentWaveFunction::PairedCFOnSpherePermanentWaveFunction()
{
  this->NbrParticles = 0;
...
//
PairedCFOnSpherePermanentWaveFunction::PairedCFOnSpherePermanentWaveFunction(int nbrParticles, int nbrLandauLevels, int nbrEffectiveFlux,
							   ...
							   bool correctPrefactors, int jastrowPower)
{
  ...
  this->PermMatrix = new ComplexMatrix(this->NbrParticles/2,this->NbrParticles/2);
...
PairedCFOnSpherePermanentWaveFunction::PairedCFOnSpherePermanentWaveFunction(const PairedCFOnSpherePermanentWaveFunction& function)
{
...
Abstract1DComplexFunction* PairedCFOnSpherePermanentWaveFunction::Clone ()
{
  return new PairedCFOnSpherePermanentWaveFunction(*this);
}
...
Complex PairedCFOnSpherePermanentWaveFunction::operator ()(RealVector& x)
{
  ...
  for (int i=0; i<NbrParticles/2; ++i)
    {
      for (int j=NbrParticles/2; j<NbrParticles; ++j)
	{
	  tmp=PermanentNorm/Orbitals->JastrowFactorElement(i,j);
	  // initialize Cauchy determinant 
	  (*PermMatrix)[i].Re(j-NbrParticles/2) = Real(tmp);
	  (*PermMatrix)[i].Im(j-NbrParticles/2) = Imag(tmp);
	}
    }
  this->Permanent = PermMatrix->Permanent();
  
  //cout << *Slater << endl;
  return this->Permanent*Slater->Pfaffian()*Interpolation*AdditionalJastrow;
}
...
Complex PairedCFOnSpherePermanentWaveFunction::CalculateFromSpinorVariables(ComplexVector& uv)
 {
...
//
Complex PairedCFOnSpherePermanentWaveFunction::GetForOtherParameters( double *coefficients)
{
  Complex tmp;	      
  // initialize Slater determinant (or Pfaffian matrix)
  for (int i=0;i<this->NbrParticles;++i)
...
void PairedCFOnSpherePermanentWaveFunction::GetForManyParameters(ComplexVector &results, RealVector& x, double **coefficients)
{
  this->OrbitalValues = (*Orbitals)(x);
  this->EvaluateTables();
  Complex tmp;
...
void PairedCFOnSpherePermanentWaveFunction::SetTrialParameters(double * coefficients)
{
...
// x = point where the function has to be evaluated
void PairedCFOnSpherePermanentWaveFunction::AdaptNorm(RealVector& x)
{
...
void PairedCFOnSpherePermanentWaveFunction::AdaptAverageMCNorm(int thermalize, int average)
{
...
void PairedCFOnSpherePermanentWaveFunction::EvaluateTables()
{
