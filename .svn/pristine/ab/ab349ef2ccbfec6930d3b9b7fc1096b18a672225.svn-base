...
int main(int argc, char** argv)
{
  ...
  OptionManager Manager ("FQHETorusBosonsWithSpinCoulomb" , "0.01");
  ...
  sprintf (OutputNameLz, "bosons_torus_su2_kysym_coulomb_n_%d_2s_%d_cyclotronenergy_%f_ratio_%f.dat", NbrParticles, MaxMomentum, Cyclotron, XRatio);
  ofstream File;
  ...
  for (; Momentum <= Max; ++Momentum)
...
#endif
	  {
	    Space = new BosonOnTorusWithSpin(NbrParticles, MaxMomentum, Momentum);	    
	  }
	else
	  {	    
	  }
	...
	Memory = Architecture.GetArchitecture()->GetLocalMemory();
      ...
      AbstractQHEHamiltonian* Hamiltonian = new ParticleOnTorusTwoLandauLevelsHamiltonian (Space, NbrParticles, MaxMomentum, Cyclotron, XRatio, HaveDelta, Architecture.GetArchitecture(), Memory);
