...
int TwoDimensionalHeisenbergModelGetLinearizedIndex(int xPosition, int yPosition, int nbrUnitCellX, int nbrUnitCellY);
...
// nbrUnitCellY = number of unit cells along the y direction
// return value = linearized index
void TwoDimensionalHeisenbergModelGet2DCoordinates(int index, int& xPosition, int& yPosition, int nbrUnitCellX, int nbrUnitCellY);
...
int main(int argc, char** argv)
{
  ...
  OptionManager Manager ("TwoDimensionalHeisenbergModel" , "0.01");
  ...
  (*SystemGroup) += new SingleDoubleOption ('\n', "j-value", "Heisenberg XX coupling constant between nearest neighbors", 1.0);
  (*SystemGroup) += new SingleDoubleOption ('\n', "jz-value", "Heisenberg Z coupling constant between nearest neighbors", 1.0);
  ...
  if (Manager.ProceedOptions(argv, argc, cout) == false)
    {
      cout << "see man page for option syntax or type TwoDimensionalHeisenbergModel -h" << endl;
      return -1;
    }
  ...
  sprintf (OutputParameterFileName + strlen(OutputParameterFileName), "j_%.6f_jz_%.6f", Manager.GetDouble("j-value"), Manager.GetDouble("jz-value"));
  if (Manager.GetBoolean("disable-momentum") == false)
  ...
  for (int i = 0; i < NbrSpins; ++i)
    {
      int XValue;
      int YValue;
      TwoDimensionalHeisenbergModelGet2DCoordinates(i, XValue, YValue, NbrSitesX, NbrSitesY);
      ...
      InversionTable[i] = TwoDimensionalHeisenbergModelGetLinearizedIndex(XValue, YValue, NbrSitesX, NbrSitesY);
...
inline int TwoDimensionalHeisenbergModelGetLinearizedIndex(int xPosition, int yPosition, int nbrUnitCellX, int nbrUnitCellY)
{
  while (xPosition < 0)
    {
...
inline void TwoDimensionalHeisenbergModelGet2DCoordinates(int index, int& xPosition, int& yPosition, int nbrUnitCellX, int nbrUnitCellY)
{
  yPosition = index;
