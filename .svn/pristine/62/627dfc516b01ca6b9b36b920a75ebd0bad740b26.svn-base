...
#ifndef SPIN1CHAINWITHINVERSIONSYMMETRY_H
#define SPIN1CHAINWITHINVERSIONSYMMETRY_H
...
//
static  unsigned long Spin1ChainWithInversionTable[] = {
...
class Spin1ChainWithInversionSymmetry : public Spin1ChainWithSzSymmetry
{
 ...
 public:
  ...
  // default constructor
  //
  Spin1ChainWithInversionSymmetry ();
  ...
  Spin1ChainWithInversionSymmetry (int chainLength, int inversionSector, unsigned long memorySize);
  ...
  // memorySize = memory size in bytes allowed for look-up table
  Spin1ChainWithInversionSymmetry (int chainLength, int inversionSector, int sz, unsigned long memory) ;

  // copy constructor (without duplicating datas)
  //
  // chain = reference on chain to copy
  Spin1ChainWithInversionSymmetry (const Spin1ChainWithInversionSymmetry& chain);
  ...
  Spin1ChainWithInversionSymmetry& operator = (const Spin1ChainWithInversionSymmetry& chain);
...
inline void Spin1ChainWithInversionSymmetry::ApplyInversionSymmetry(unsigned long& stateDescription)
{
#ifdef __64_BITS__
  unsigned long InitialState = stateDescription & 0xfffffffffffffffful;  
#else
  unsigned long InitialState = stateDescription & 0xfffffffful;  
#endif	
  InitialState <<= this->InversionShift;
  //  cout << InitialState << endl;
#ifdef __64_BITS__
  unsigned long TmpState = Spin1ChainWithInversionTable[InitialState & 0xfful] << 56;
  TmpState |=Spin1ChainWithInversionTable[(InitialState >> 8) & 0xfful] << 48;
  TmpState |=Spin1ChainWithInversionTable[(InitialState >> 16) & 0xfful] << 40;
  TmpState |=Spin1ChainWithInversionTable[(InitialState >> 24) & 0xfful] << 32;
  TmpState |=Spin1ChainWithInversionTable[(InitialState >> 32) & 0xfful] << 24;
  TmpState |=Spin1ChainWithInversionTable[(InitialState >> 40) & 0xfful] << 16;
  TmpState |=Spin1ChainWithInversionTable[(InitialState >> 48) & 0xfful] << 8;
  TmpState |=Spin1ChainWithInversionTable[InitialState >> 56]; 
#else
  unsigned long TmpState =Spin1ChainWithInversionTable[InitialState & 0xfful] << 24;
  TmpState |=Spin1ChainWithInversionTable[(InitialState >> 8) & 0xfful] << 16;
  TmpState |=Spin1ChainWithInversionTable[(InitialState >> 16) & 0xfful] << 8;
  TmpState |=Spin1ChainWithInversionTable[InitialState >> 24];
#endif	
...
inline unsigned long Spin1ChainWithInversionSymmetry::FindCanonicalForm(unsigned long stateDescription, double& szSymmetrySign)
{
...
inline int Spin1ChainWithInversionSymmetry::FindOrbitSize(unsigned long stateDescription)
{
...
inline bool Spin1ChainWithInversionSymmetry::TestDiscreteSymmetryConstraint(unsigned long stateDescription)
{
  unsigned long TmpStateDescription = stateDescription;
  this->ApplyInversionSymmetry(TmpStateDescription);
